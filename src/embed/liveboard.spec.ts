...
describe('Liveboard/viz embed tests', () => {
    ...
    test('Should set liveboard options', async () => {
        ...
        await executeAfterWait(() => {
            expectUrlMatchesWithParams(
                getIFrameSrc(),
                `http://${thoughtSpotHost}/?embedApp=true${defaultParams}&preventPinboardFilterRemoval=true&isLiveboardEmbed=true&isPinboardV2Enabled=true&enableAskSage=false#/embed/viz/${liveboardId}/tab/${activeTabId}`,
            );
        });
    });
