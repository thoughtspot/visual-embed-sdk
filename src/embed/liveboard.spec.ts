...
describe('Liveboard/viz embed tests', () => {
    ...
    test('should set lazyLoadingForFullHeight to true in url when fullHeight is true', async () => {
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: true,
        ...
        await executeAfterWait(() => {
            expectUrlMatchesWithParams(
                getIFrameSrc(),
                `http://${thoughtSpotHost}/?embedApp=true${defaultParams}&fullHeight=true&lazyLoadingForEmbed=true${prefixParams}#/embed/viz/${liveboardId}`,
            );
        });
    });

    test('should not set lazyLoadingForFullHeight in url when fullHeight is false', async () => {
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: false,
            lazyLoadingForFullHeight: true,
    ...
    test('should not set lazyLoadingForFullHeight in url when lazyLoadingForFullHeight is false', async () => {
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: true,
        ...
        await executeAfterWait(() => {
            expectUrlMatchesWithParams(
                getIFrameSrc(),
                `http://${thoughtSpotHost}/?embedApp=true${defaultParams}&fullHeight=true${prefixParams}#/embed/viz/${liveboardId}`,
            );
        });
    });

    test('should register RequestFullHeightLazyLoadData event handler when lazyLoadingForFullHeight is true', async () => {
        const onSpy = jest.spyOn(LiveboardEmbed.prototype, 'on');
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: true,
        ...
        executeAfterWait(() => {
            expect(onSpy).toHaveBeenCalledWith(EmbedEvent.RequestFullHeightLazyLoadData, expect.anything());
        });
    });

    test('should not register RequestFullHeightLazyLoadData event handler when fullHeight is false', async () => {
        const onSpy = jest.spyOn(LiveboardEmbed.prototype, 'on');
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: false,
        ...
        executeAfterWait(() => {
            expect(onSpy).not.toHaveBeenCalledWith(EmbedEvent.RequestFullHeightLazyLoadData, expect.anything());
        });
    });

    test('should call sendFullHeightLazyLoadData and trigger FullHeightLazyLoadData host event', async () => {
        const liveboardEmbed = new LiveboardEmbed(getRootEl(), {
            ...defaultViewConfig,
            fullHeight: true,
            lazyLoadingForFullHeight: true,
        ...
        const triggerSpy = jest.spyOn(liveboardEmbed, 'trigger');
        // Mock the calculateVisibleElementData function from utils
        const utils = await import('../utils');
        const calculateVisibleElementDataSpy = jest.spyOn(utils, 'calculateVisibleElementData')
            .mockReturnValue({ mockData: 'test' });

        await liveboardEmbed.render();

        // Call the sendFullHeightLazyLoadData method directly
        liveboardEmbed.sendFullHeightLazyLoadData();

        expect(calculateVisibleElementDataSpy).toHaveBeenCalledWith(liveboardEmbed.el);
        expect(triggerSpy).toHaveBeenCalledWith(HostEvent.FullHeightLazyLoadData, { mockData: 'test' });
    });
