...
describe('startAutoMCPFrameRenderer', () => {
    ...
    describe('getMCPIframeSrc URL construction', () => {
        test('should strip tsmcp param and merge embed params into rendered src', async () => {
            ...
            // tsmcp must not survive on the rendered iframe: isTSMCPIframe would
            // otherwise re-match the SDK's own output and cause an infinite
            // replacement loop via the same MutationObserver.
        ...
        test('should mark the rendered iframe with isMcpAnswerEmbed=true instead of tsmcp', async () => {
            let capturedSrc = '';
            renderIFrameSpy.mockRestore();
            renderIFrameSpy = jest.spyOn(TsEmbed.prototype as any, 'renderIFrame')
                .mockImplementation(async function (this: any, src: string) {
            ...
            const observer = startAutoMCPFrameRenderer();
            const iframe = document.createElement('iframe');
            iframe.src = `https://${thoughtSpotHost}/v2/?${Param.Tsmcp}=true#/embed/answer`;
            document.body.appendChild(iframe);
            await new Promise((r) => setTimeout(r, 50));

            // The app-facing marker is present, but tsmcp (the SDK's own
            // re-processing trigger) is not — this is what keeps the
            // MutationObserver from looping on its own rendered output.
            expect(capturedSrc).toContain(`${Param.IsMcpAnswerEmbed}=true`);
            expect(capturedSrc).not.toContain(`${Param.Tsmcp}=true`);
            observer.disconnect();
        });
