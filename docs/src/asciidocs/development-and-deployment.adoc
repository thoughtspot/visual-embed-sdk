...
GUIDs are randomly generated when an object is first created. If you need to track relationships between objects and you require that objects all have the same display name, you should store the relationships between various GUIDs for later reference at the time you first publish the child objects. For example, if you want to know which liveboards are copies from a particular template, store the returned GUIDs of the child versions when you first publish them, so that you can push future updates from the parent liveboard to the descendents.
