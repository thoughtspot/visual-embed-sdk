...
This page explains how to embed a single ThoughtSpot visualization in your Web page, portal, or application. A visualization is one single answer on a Liveboard, displayed without the search bar or other options to modify the contents of the visualization.

You cannot embed a saved answer as an individual visualization. Saved answers can be used in the *SearchEmbed* component, where they will load with the search bar and other options of ThoughtSpot search. Any visualization you want to show via *LiveboardEmbed* must exist on a Liveboard object in ThoughtSpot.
...
----
import {
	LiveboardEmbed, AuthType, init, prefetch, EmbedEvent
}
...
< script type = 'module' >
	import {
		LiveboardEmbed, AuthType, init, prefetch
	}
