...
|`User-Agent`|For most REST clients, the `User-Agent` header is added by default in the API requests. If the requesting user agent is a .NET client, you must set the `User-Agent` header in all API requests. You can specify any string that identifies the client; for example, `'User-Agent: <browser>/<browser-version><os/platform>'`.
