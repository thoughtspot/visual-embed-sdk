...
Multipart/Form-data::
For some APIs, you must send the request body as separate blocks of data or as a JSON file in `POST` requests. For APIs that require multiple blocks of data, make sure you set the `Content-Type` as `multipart/form-data`. To know if an API uses `multipart/form-data` content-type, see the example requests on the xref:rest-api-reference.adoc[respective API reference page]. 
