...
* Using the *REST Playground v1* menu option in the Developer portal (Requires Developer or Admin privilege)
+
...
For more information about the authentication methods and the REST v1 endpoints, see xref:api-auth-session.adoc[] for user authentication and session management, see 
...
Some endpoints may require you to send a `POST` request for delete or update operation. For more information about the endpoint URIs and request methods, refer to the xref:rest-api-reference.adoc[REST API v1 Reference Guide].
...
=== Request parameters
Some API endpoints allow you to send attributes as query parameters in `POST` requests. For example, the  `/tspublic/v1/metadata/listobjectheaders` API endpoint requires query parameters that are passed as a query string in the URL.
...
image::./images/api-v2-error.png[REST API v2 error codes]
