...
:page-description: You can insert custom buttons or menu items in ThoughtSpot UI to let your application users to analyze insights and trigger an action on the data.
...
ThoughtSpot also allows inserting custom buttons and menu actions. If you want to push business insights from ThoughtSpot to an external app or a specific URL, you can use the custom actions feature. In embedded instances, ThoughtSpot also allows adding a custom action to trigger a callback event to the host application and initiate an action.
...
<h5>URL actions</h5>
<p class="boxBody">
Push data to specific web page or URL destination</p>
...
<p class="boxBody"><em>Requires access to the *Custom actions* panel</em>.</p>
...
<p class="boxBody"><a href="?pageid=push-data">Learn more -></a></p>
</div>
</a>
</div>
