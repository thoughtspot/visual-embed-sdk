...
ThoughtSpot also allows adding custom buttons and menu actions. If you want to push business insights from ThoughtSpot to an external app or a specific URL, you can use the custom actions feature. In embedded instances, ThoughtSpot also allows adding a custom action to trigger a callback event to the host application and initiate an action.
