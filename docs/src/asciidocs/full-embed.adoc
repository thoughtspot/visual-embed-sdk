...
For example, to disable the *download* action from the *More* (*...*) menu on the Liveboard or visualization page, specify the `Download` action menu string in the `disabledActions` attribute.
...
For example, if the custom action ID is set as *send-email*, use `send-email` in the `disabledActions` attribute to disable this action on a Liveboard or answer page.
====

disabledActionReason::
_String_. Reason for disabling an action on a Liveboard or visualization page. To display a tooltip for the disabled menu item, specify the reason for disabling the menu item in the `disabledActionReason` attribute.
...
hiddenActions::
_Array of strings_. The action menu items to hide on a Liveboard or visualization page.
+
...
For example, if the custom action ID is set as *send-email*, use `send-email` in the `hiddenActions` attribute to hide this action on a Liveboard or answer page.
====
...
* `*Page.Liveboards*`
+
Displays the *Liveboards* page when the application loads.
