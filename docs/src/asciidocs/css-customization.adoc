...
. To allow your instance to use an external stylesheet, your instance must allow a CSP override by setting the `style-src` property. Contact ThoughtSpot Support to enable this setting on your cluster.
...
+
The following example shows how to add a custom font and apply it in the CSS file:
...
=== Search bar and data panel elements
...
* Primary +
Buttons that trigger an action for an entire page or a container. For example, the Go button on the search bar.  Primary buttons may contain text, text with an icon, or just an icon.
...
* Tertiary
Buttons that are less prominent and trigger independent actions for sub-tasks on a page. For example, the Undo, Redo, Reset buttons on the search bar.
...
|======
|Variable|Description
|`--ts-var-menu-color`|Font color of the text on menu panels.
|`--ts-var-menu-background`|Background color of menu panels.
|`--ts-var-menu-font-family`|Font family specification for the text on menu panels.
|`--ts-var-menu-text-transform`|Text capitalization specification for menu options.
