...
The `rules` option in the `customCSS` object allows you to apply style overrides to the UI components and elements that cannot be customized using the variables provided by ThoughtSpot. While the `rules` option allows granular customization of individual elements, you must exercise caution because the rule-based style overrides may break when your ThoughtSpot instance is upgraded to a new release version.

When applying style overrides using `rules`, make sure you use the correct style class and values in your rule statements. To find the class name of an element, select the element and click *Inspect* on your browser. The *Elements* tab in the Developer Tools console shows the style classes for the selected element.

The following example shows how to change the background color of the *All Tags* and *All Authors* dropdowns that appear on the *Home* page of the ThoughtSpot application. Make sure you add the `_UNSTABLE` suffix to the `rules` property.
...
The following figure shows the style override applied using `rules`:
[.widthAuto]
[.bordered]
