...
By default, ThoughtSpot will display a login box with a link at the bottom that says "Login using Single Sign On", even when embedding using the Visual Embed SDK. To make the SSO process seamless, the ThoughtSpot Server must be configured to automatically redirect to the SAML IdP. To enable this feature, contact ThoughtSpot Support.
