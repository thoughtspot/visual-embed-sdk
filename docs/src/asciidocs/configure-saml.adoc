...
==== Configuring ThoughtSpot for automatic SAML redirect
By default, ThoughtSpot will display a login box with a link at the bottom that says "Login using Single Sign On", even when embedding using the Visual Embed SDK. To make the SSO process seamless, the ThoughtSpot Server must be configured to automatically redirect to the SAML IdP. 

The necessary configuration below can be run by your own ThoughtSpot system administrator or you can make a support request to have it enabled:

`tscli --adv service add-javaopt tomcat.tomcat D orion.samlConfiguration.samlLoginPageRedirect true`

Once automatic SAML redirect has been enabled, you can override it when necessary by appending the following parameter to a ThoughtSpot URL:
...
==== Configuring IdP to allow iframe embedding
The Visual Embed SDK creates an iframe to display the embedded ThoughtSpot content. By default, most IdPs do not allow a SAML workflow from an iframe embed. To enable a fully seamless SSO experience for your end users, you will need to have your IdP configured to allow an iframe to complete the SAML workflow. 
