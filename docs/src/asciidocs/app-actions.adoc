...
ThoughtSpot automates and simplifies the workflow with app actions and secures data transactions between the source and destination apps with OAuth authentication and authorization. App actions allow ThoughtSpot to automatically establish a pipeline for data exchange and thus eliminate the need for building connectors to external apps. Your application users can quickly get started with an app action and push data to their workspace with a few simple steps.
...
* In the 8.1.0.cl release version, ThoughtSpot supports app actions for Slack integration only. 
...
* By default, a global app action is added as a menu action in the **More** menu image:./images/icon-more-10px.png[the more options menu].
