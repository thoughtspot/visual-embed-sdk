...
To get the visualization GUID of a given visualization on a Liveboard, right-click the More (...) menu for that visualization and click "Copy embed link". The link will be in the format `{thoughtSpotServer}/#/embed/viz/{liveboardGuid}/{visualizationGuid}`.
...
The JSON object created by the code above will match the format documented in the xref: xref:pinboarddata.adoc[Liveboard data API].  
...
The `LiveboardData` class matches to the Liveboard data API. The `LiveboardData.vizData[{vizId}]` object brings back all of the data for the given visualization. You can retrieve all of the data for a given column using the `.data[{columnName}]` object of the return from `.vizData`:
...
== Render the chart
Once you have transformed the data into the input format, you should be able to render the chart into the page. Your overall steps will look like the following:
