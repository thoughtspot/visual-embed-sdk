...
ThoughtSpot also has a default group called `All`. When you create new users in ThoughtSpot, they are automatically added to `All`.

By default, the members of `All` do not have the permission to download or upload data. You can use the Group APIs to globally add the following privileges to all the users in `All`.
...
|`groupNames`|string|A JSON array of group names to which you want to add the privilege. To add a privilege to all user groups, specify `All`.
|====
...
|`groupNames`|string|A JSON array of group names. To remove the privilege from all user groups, specify `All`.
|====
...
[source, cURL]
----
curl -X POST --header 'Content-Type: application/x-www-form-urlencoded' --header 'Accept: application/json' --header 'X-Requested-By: ThoughtSpot' -d 'privilege=USERDATAUPLOADING&groupNames=All' 'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/group/removeprivilege'
----
