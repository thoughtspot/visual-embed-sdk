...
* *Get data* button
+
The *Get data* button triggers the `data` embed event (`EmbedEvent.Data`), which returns the Answer data. Your host application can use this raw data as a payload and render it the way you want.

* Data panel +
The data sources panel allows users to choose the data sources to search from, and apply formulas and filters. The current version of the SDK doesn't allow you to hide or collapse the data panel.
