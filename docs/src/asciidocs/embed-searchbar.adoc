...
const getQueryResult = async (query: string, sourceName: string) => {
  ...
  //Fetching data for given query and source using TS public api
  ...
  return {
    colNames: data.contents[0].column_names,//Parsing data to get column names
    rows: data.contents[0].data_rows,//Parsing data to get rows
  };
};

tsembed.on(EmbedEvent.GetDataClick, ()=>{
     //On GetDataClick, trigger GetTML hostEvent to get query and datasource for api call
