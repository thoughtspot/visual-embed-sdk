...
+
By default, the new Liveboard experience[beta betaBackground]^Beta^ is turned off on embedded instances. You can enable it using the `liveboardV2` setting in the `LiveboardEmbed` SDK. However, ThoughtSpot recommends using classic Liveboard experience on embedded apps until the new Liveboard experience is GA.

==== Liveboard layout in embedded mode
In the new Liveboard experience[beta betaBackground]^Beta^ mode, the embedded Liveboard resizes to one-column layout if the screen width is 1024px or less. In classic experience mode, the embedded Liveboard adjusts its layout to two-column or one-column view to fit the screen width and orientation of the user's device.
