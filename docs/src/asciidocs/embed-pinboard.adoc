...
< script type = 'module' >
	import {
		PinboardEmbed, AuthType, init, prefetch
	}
...
disabledActions::
_Array of strings_. The actions that must appear as disabled on the Pinboards page.
+
...
++++
+
For a complete list of actions and the corresponding strings, see link:{{visualEmbedSDKPrefix}}/enums/action.html[Action, window=_blank].
...
hiddenActions::
_Array of strings_. The actions to hide on the Pinboards page.
+
