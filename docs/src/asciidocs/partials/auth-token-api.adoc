...
Starting from 8.9.0.cl, the `/tspublic/v1/session/auth/token` API endpoint supports provisioning users Just-in-Time (JIT), mapping the user to groups, and dynamically assign privileges. If the user for whom you are requesting an authentication token doesn't exist in the ThoughtSpot system, you can set the `autocreate` property and assign the user to groups in the same API call.
