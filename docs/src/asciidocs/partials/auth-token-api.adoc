...
Starting from 8.9.0.cl, the `/tspublic/v1/session/auth/token` API endpoint supports Just-in-Time (JIT) provisioning of a user. If the user you specify in the API request doesn't exist in the ThoughtSpot system, you can create the user and assign the user to groups by using the `autocreate` and `groups` attributes in the same API call.
...
* If the specified user does not exist in ThoughtSpot, and the `autocreate` property is set to `true`, the user is created and mapped to the groups specified in the `groups` attribute.
* If the user already exists in ThoughtSpot, but `autocreate` is set to `true` and the group names are defined in the `groups` attribute, the user will be added to the groups specified in the API request.
====
