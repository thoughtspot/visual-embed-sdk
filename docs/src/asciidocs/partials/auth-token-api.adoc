If you have enabled xref:trusted-authentication.adoc#trusted-auth-enable[trusted authentication] on ThoughtSpot and have access to the `secret key`, you can send an API request to the `/tspublic/v1/session/auth/token` endpoint and get a login token for a ThoughtSpot user. The
...
The `/tspublic/v1/session/auth/token` API endpoint supports Just-in-Time (JIT) provisioning of a user. If the user you specify in the API request doesn't exist in the ThoughtSpot system, you can create the user and assign the user to groups by using the `autocreate` and `groups` attributes in the same API call.
...
* If the user exists in ThoughtSpot but not assigned to any Org yet and `autocreate` is set to `true`, the user will be added to the specified Org.
...
* If the user doesn't exist in ThoughtSpot and `autocorrect` is set to `true`, the user is assigned to the specified groups.
...
[NOTE]
====
* If the username specified in the API request exists in the ThoughtSpot system, the API returns the token for logging in the user, else it returns the `user not found error`.
* If the specified user does not exist in ThoughtSpot, and the `autocreate` property is set to `true`, the user is created and mapped to the groups specified in the `groups` and Org ID specified in the `orgid` attributes.
* If the user already exists in ThoughtSpot, but `autocreate` is set to `true` and the group names are defined in the `groups` attribute, the user will be added to the groups specified in the API request. Similarly, if you have specified the `orgid` and set `autocreate` to `true`, the user will be assigned to the specified Org.
