...
Starting from 8.9.0.cl, the `/tspublic/v1/session/auth/token` API endpoint supports Just-in-Time (JIT) user creation and dynamic group mapping and privilege assignment to a user. If the user you specify in the API request doesn't exist in the ThoughtSpot system, you can create the user and assign the user to groups by using the `autocreate` and `groups` attributes in the same API call.
