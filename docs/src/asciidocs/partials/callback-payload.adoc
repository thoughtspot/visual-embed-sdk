...
If the dataset is divided into 10 batches, and the offset value is `0` and the batchSize is `3`, the first three batches of the dataset is retrieved in the answer payload in one UI query. Similarly, if the offset value is `1` and the batchSize is `3`, the next set of batches (4,5,and 6) are retrieved.
