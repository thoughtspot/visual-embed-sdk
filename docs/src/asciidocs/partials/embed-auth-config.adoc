...
None;;
Allows using your existing IdP setup and authentication configuration on ThoughtSpot to authenticate an embedded application user.
If you want to use pass-through authentication, ThoughtSpot recommends using `AuthType.Passthrough` instead of `AuthType.None`.

Passthrough;;
Enables pass-through authentication. Passthrough authentication requires minimal configuration, because it uses your existing IdP setup and the current SAML or OIDC configuration on ThoughtSpot to allow access to the embedded content.
...
+
For more information, see xref:embed-authentication.adoc#trusted-auth-embed[Embed user authentication].
