...
**How to fix**: Use the xref:embed-authentication.adoc#trusted-auth-embed[`getAuthToken` function] in the SDK to return a fresh token for each re-login.
...
For example, if `Store Region` is set to `west, southwest` in the Liveboard filter, and you set runtime filter of `Store Region` as `east`, you will get the *No data found* error because the total filter condition is `Store Region IN ('west', 'southwest') AND Store Region = 'east'`, which results in an emtpy data set.
