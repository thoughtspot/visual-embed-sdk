...
The promise returned resolves to  a string that contains the transient Liveboard content, which is encoded as JSON and is sent to the `/tspublic/v1/export/pinboard/pdf` endpoint with the `transient_pinboard_content` key. This content resembles the current Liveboard as is, including the unsaved changes to the following:

* Liveboard filters
* Liveboard layout
* If the new Liveboard experience is enabled, the transient content includes ad hoc changes to the visualization content such as sorting,  toggling of legends, and data drill down.
...
<script src="/path/to/iveboard.js"></script>
< script >
