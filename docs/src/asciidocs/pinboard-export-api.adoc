...
== Request Parameters+++<table>++++++<colgroup>++++++<col style="width:20%">++++++</col>+++
   ...
   +++<tbody>++++++<tr>++++++<td>++++++<code>+++id+++</code>++++++</td>+++
         ...
         +++<td>+++None+++</td>++++++</tr>+++
     +++<tr>++++++<td>++++++<code>+++layout_type+++</code>++++++</td>+++
      ...
      +++<tr>++++++<td>++++++<code>+++orientation+++</code>++++++</td>+++
         ...
         +++<td>+++Yes+++</td>+++
         +++<td>+++LANDSCAPE+++</td>++++++</tr>+++
      +++<tr>++++++<td>++++++<code>+++truncate_tables+++</code>++++++</td>+++
      +++<td>+++boolean+++</td>+++
...
{% include note.html content="These parameters are case-insensitive. `EQ`, `eq`, and `eQ`, for example, have the same result." %}
...
If your ThoughtSpot environment is embedded, and you want to download Pinboards with unsaved changes as PDFs, you must use the link:{{ site.baseurl }}/reference/api/about-JS-API.html[JavaScript API] and pass the `transient_pinboard_content` parameter in the browser fetch request, using the `getExportRequestForCurrentPinboard` method.
