...
If your ThoughtSpot instance is set as a multi-tenant application and is logically partitioned into organizations, the cluster administrators can send a `PUT` request to the  `/tspublic/v1/session/org/` endpoint to switch the organization context.

For example, to perform configuration changes at the `All org` level, the cluster administrator must switch to `org -1`.
Similarly, before performing CRUD operations on the objects of a specific organization, the cluster administrator must switch the session to the relevant org context.

=== Resource URL
----
PUT /tspublic/v1/session/org/
----
...
|`orgid`|__Integer__. ID of the org object. A value of `-1` indicates the `All Org` scope and `0` refers to the primary organization.
|===
=== Example request
...
--header 'Accept: application/json' \
-d 'orgid=-1' \
'http://<ThoughtSpot-host>/callosum/v1/tspublic/v1/session/org/'
----

.Request URL
----
https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/session/org/
----
...
|`userid` __Optional__|__String__. GUID of the user account for which you want to set the home Liveboard. If the `userid` attribute is not defined, ThoughtSpot sets the home Liveboard of the current logged-in user.
|===
=== Example request
