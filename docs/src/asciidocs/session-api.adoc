...
[#orgSwitch]
== Switch between Orgs
If your ThoughtSpot instance has multiple Orgs, you can switch between Orgs by using the `/tspublic/v1/session/org/` endpoint.
...
|`orgid`|__Integer__. ID of the org object. The org ID `0` indicates `Primary Org`.
|===
=== Example request
