...
[#orgSwitch]
== Switch between Orgs
If your ThoughtSpot instance is set as a multi-tenant application and is logically partitioned into organizations, the cluster administrators can send a `PUT` request to the  `/tspublic/v1/session/org/` endpoint to switch to another Org context.
...
|`orgid`|__Integer__. ID of the org object. A value of `-1` indicates the `All Org` scope and `0` refers to the `Primary Org`.
|===
=== Example request
