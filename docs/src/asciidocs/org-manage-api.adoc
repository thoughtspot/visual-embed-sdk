...
. For Orgs CRUD operations, use the xref:org-api.adoc[`/tspublic/v1/org/`] endpoints. For all Org operations, you must set the org scope to `ALL` in each of your API requests and pass it as a query parameter in the request URL.
...
[options='header']
|=====
|Operation category| API endpoints to use
