...
Available with `LiveboardEmbed` and `AppEmbed` in `V1.15.0` or later. +
Triggers the *Copy link* action on a Liveboard or a specific visualization on a Liveboard.
...
* `points` +
Data points to drill down to. Returns clicked and selected data points of a visualization object.
...
Triggers the *Edit* mode on a Liveboard. To trigger the *Edit* action for a specific visualization on a Liveboard, specify the visualization GUID.
...
Is triggered when a user navigates to an application page without reloading the page. This event is the same as  `appEmbed.navigateToPage(path, true)` function.

Requires `path` definition to allow navigation to back or forward.
...
* `searchQuery`  __String__ +
The search query string

* `dataSourceIds` __String__ +
Data source GUIDs
...
Triggers an event to update runtime filters. You can specify an array runtime filters with the following attributes:
