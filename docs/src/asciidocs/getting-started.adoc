...
Call the `init` method as early as possible and complete authentication before the embedded content loads.

Prefetch and cache resources::

Use the `prefetch` method in the SDK to xref:prefetch-and-cache.adoc[prefetch and cache] static resources required for loading the embedded components. Make sure you call prefetch in the `init` method before login process and cache static assets as early as possible.
