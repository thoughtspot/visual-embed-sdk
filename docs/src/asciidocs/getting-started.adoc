...
== Adjust CORS and CSP allowed domains in Security Settings
ThoughtSpot is configured by default to only allow embedding and REST API commands from specified domains. 

If you encounter any errors with using the Visual Embed SDK components, first ferify if your host application domain is xref:security-settings.adoc[whitelisted for CORS and CSP].

== Enable Developer Portal access
The Developer Portal is accessed via "DEVELOP"in the top ThoughtSpot menu.

Only Administrators and users who belong to a group with the privlege called "Has Developer privilege" can see the "DEVELOP" menu link. 

Please have your administrator add any users who will be developing embedded solutions to a group with "Has Developer privilege"
