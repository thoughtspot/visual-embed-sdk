...
The following example shows classes and functions for handling Liveboard data payload triggered from a callback action in the **More** options menu.
...
class TabularData {
    ...
    constructor(originalData = null) {
        ...
        this.data = {}; // The data is normalized to be stored by column with the key being the column name.  This
    ...
    class LiveboardActionData extends TabularData {
        //Creates a new LiveboardActionData object from JSON.
        static createFromJSON(jsonData) {
            let isV1 = true;
            // Handle data structure differences between Liveboard v1 and v2.
...
For more examples, see code samples in `dataclasses.js` on link:https://github.com/thoughtspot/ts_everywhere_resources/blob/master/apis/dataclasses.js[ThoughtSpot Everywhere Resources GitHub repository, window="_blank"].
