...
The format of the data payload for Liveboards varies if the callback action is triggered from a Liveboard in the experience mode. 

To view the payload structure, refer to the examples on xref:callback-response-payload.adoc[Custom action response payload] page.
...
class LiveboardActionData {
    ...
    * Liveboard actions pass the JSON as a string, so it has to be
        *
        converted first.*@param jsonData A string from the payload.data.*@returns {
            LiveboardActionData
        }
        */
