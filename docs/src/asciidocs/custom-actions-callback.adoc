...
The format of the data payload for Liveboard visualization varies if the callback action is triggered from a Liveboard in the experience mode. To view the payload structure, refer to the examples on xref:callback-response-payload.adoc[Custom action response payload] page.
