...
The upcoming release introduces new REST v1 endpoints and object properties on clusters that have the Orgs [beta betaBackground]^Beta^ feature enabled.

Org API endpoints::
For multi-tenant clusters with Orgs feature enabled, ThoughtSpot provides new endpoints to allow CRUD operations on Org objects.
+
...
If the Orgs feature is enabled on your cluster and Orgs are created, the cluster administrators can use the `/tspublic/v1/session/orgs` to xref:session-api#orgSwitch[Switch between Orgs].

+
For deployments with trusted authentication framework, you can use the `/tspublic/v1/session/auth/token` API endpoint  xref:session-api.adoc#session-authToken[to create a user just-in-time and dynamically assign Orgs, groups, and privileges to the user].
+
For more information, see xref:session-api.adoc#orgSwitch[Switch between Orgs].

Mapping Orgs to users and groups::
ThoughtSpot clusters with Orgs feature support mapping users and groups to Orgs.
