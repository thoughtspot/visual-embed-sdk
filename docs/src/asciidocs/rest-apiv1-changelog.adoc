...
The POST /tspublic/v1/user/ and PUT /tspublic/v1/user/{userid} API endpoints allow you set the `triggeredbyadmin` to indicate if the user creation or update request is initiated by the ThoughtSpot admin or an external application.
