...
ThoughtSpot now supports Just-in-time (JIT) provisioning of users and assigning users to groups with the trusted authentication method. The xref:session-api.adoc#session-authToken[/tspublic/v1/session/auth/token] endpoint includes the `autocreate` and `groups` properties to allow the authenticator service administrator to create a user and assign the user to groups when requesting a login token via REST API.
