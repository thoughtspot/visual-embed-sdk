...
ThoughtSpot visualizations, Liveboards, and saved search answers support several actions and menu commands for various user-initiated operations. These actions appear as primary buttons, menu commands in the *More* menu image:./images/icon-more-10px.png[the more options menu] or contextual menu that appears when a user right-clicks on a visualization in a Liveboard or saved answer page. When you embed a ThoughtSpot Liveboard, visualization, or the entire application experience in another app, xref:embed-actions.adoc#standard-actions[these actions] will be available to all users by default. However, your application users may not require access to all the available menu actions, or you may want to restrict access to certain features based on the user persona or group privileges. 
...
visibleActions::
Allows you to define a list of actions that can be made visible to application users. If your embedded instance requires only a few actions, use this API to add only those actions to the embedded UI.
...
* Use the appropriate action strings to show or hide a UI action. For a comprehensive list of UI actions and the action strings in the SDK, see xref:embed-actions.adoc#standard-actions[Supported actions].  
...
If the `visibleActions` attribute is set as an empty array and the action strings are specified in `hiddenActions` attribute, no actions will appear in the embedded UI.
...
|`Action.ScheduleList`| `LiveboardEmbed`

`AppEmbed` | *Manage schedules* 
 
 Allows scheduling and monitoring Liveboard jobs.
...
[IMPORTANT]
====
The *Follow* action is deprecated in the ThoughtSpot 8 Cloud release and is replaced with the **Schedule ** button in the UI. ThoughtSpot users can now use this button to schedule Liveboard notifications. Due to this change, the `Action.Subscription` enumerator is  deprecated in the Visual Embed SDK 1.6.0 and later versions.
====
...
|`Action.ShareViz`|`AppEmbed`|*ShareViz*
...
|`Action.Pin`|`AppEmbed`|*Pin*

Pins the answer to a Liveboard.

|`Action.AnalysisInfo`|`SearchEmbed`

`AppEmbed`|*Query details*

Displays information about the search query such as attributes, measures, and options to view query visualizer and SQL statements.
