...
////
disabledActions::
Allows you to define a list of actions that must appear as disabled in the UI. You can also specify a reason for disabling the action. For example, your application user may require a specific privilege to schedule a Pinboard job, so you may want to disable this action and specify a reason, such as `Contact your administrator to enable this feature`.
...
* If you are configuring `visibleActions` in the AppEmbed SDK package, make sure to include all the actions that you want to show on **Pinboards**, **Visualizations**, and the *Saved Answers* page.
...
By default, ThoughtSpot provides a set of standard actions for embedded objects such as xref:embed-actions.adoc#pinboard-action[pinboards], xref:embed-actions.adoc#viz-actions[pinned visualizations], and xref:embed-actions.adoc#answer-actions[answers derived from the search]. The availability of these actions may vary based on the SDK library used for embedding and the user privileges.

[#pinboard-action]
=== For Pinboards

The following actions are available for the embedded Pinboard objects:

[width="100%" cols="3,2,4"]
[options='header']
|===
|Action string in SDK| Required SDK library|Action label in the UI
|`Action.AddFilter` a| `PinboardEmbed`

`AppEmbed` |*Add filters*

Adds filters to visualizations in a Pinboard.
...
Applies filters to a visualization in a Pinboard.

|`Action.Share`| `AppEmbed`  |*Share*

Allows sharing a Pinboard with another user or user group.

|`Action.Schedule`| `AppEmbed` |**Schedule**

Allows scheduling Pinboard notifications.

|`Action.Subscription`| `AppEmbed` |**Follow**

Allows subscribing and scheduling Pinboard notifications.

|`Action.Remove` | `AppEmbed`| *Delete*
Deletes a Pinboard.
|`Action.MakeACopy` | `PinboardEmbed`

`AppEmbed`| *Make a copy*

Creates a copy of the Pinboard.

|`Action.DownloadAsPdf` |  `PinboardEmbed`

`AppEmbed`|*Download as PDF*

Downloads the Pinboard as a PDF file.
|`Action.SchedulesList`| `PinboardEmbed`

`AppEmbed` | *Manage schedules*

 Allows you to manage Pinboard jobs.
|`Action.Present`| `PinboardEmbed`

`AppEmbed` | *Present*

 Presents the Pinboard in fullscreen mode.

|`Action.Save`|`PinboardEmbed`

`AppEmbed` | *Save*

 Saves Pinboard modifications.
 |`Action.PinboardInfo`|`PinboardEmbed`

`AppEmbed` |
 *Pinboard info*

 Displays information about the Pinboard.|

`Action.EditTML`|`AppEmbed` | *Edit TML*

Allows editing the ThoughtSpot Modelling Language (TML) representation of a Pinboard object loaded on the ThoughtSpot server.
|`Action.ImportTML`|`AppEmbed` | *Import TML*

Imports the TML representation of a Pinboard object to the ThoughtSpot server.
|`Action.ExportTML`|`AppEmbed` | *Export TML*

Exports the TML representation of a Pinboard object from the ThoughtSpot server.
|`Action.UpdateTML`|`AppEmbed` | *Update TML*
Publishes the modified TML representation of a Pinboard object.

|`Action.RequestAcess`| `PinboardEmbed`

`AppEmbed`|
*Request access*

Allows you to request access to edit a Pinboard. The **Pinboards **page shows a locked icon if a user does not have edit access to a Pinboard. A user can request edit access to a Pinboard by clicking the locked icon.
...
[#viz-actions]
=== For visualizations on a Pinboard

The following actions are available for ThoughtSpot visualizations pinned to a Pinboard:

[width="100%" cols="3,2,4"]
[options='header']
|===
|Action string in SDK| Required SDK library|Action label in the UI
|`Action.Explore` a| `PinboardEmbed`
...
Removes the visualization from the Pinboard.
|
`Action.ToggleSize`|`PinboardEmbed`

`AppEmbed`| The *Resize* icons in the **More** actions menu.

Allows resizing a visualization on a Pinboard.
|
`Action.DrillDown`|`PinboardEmbed`
