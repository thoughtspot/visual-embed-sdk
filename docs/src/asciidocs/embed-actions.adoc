...
disabledActions::
Allows you to define a list of actions that will appear as disabled. You can also specify a reason for disabling the action. For example, your application user may require a specific privilege to schedule a Liveboard job, so you may want to disable this action and specify a reason, such as `Contact your administrator to enable this feature`.
