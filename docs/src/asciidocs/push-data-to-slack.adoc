...
. The user clicks **Allow** to authorize the request. 
...
+
ThoughtSpot requests access to your Slack workspace and opens the app authorization page. This step is required only if you are using the Slack integration action for the first time or when ThoughtSpot refreshes its OAuth access token.
...
. Notice that the app authorization page lists your Slack workspaces by default. Select a workspace from the drop-down and then click **Allow**. 
+
If your workspace is not displayed in the list, click *Add another workspace* and complete the steps to add the workspace.
