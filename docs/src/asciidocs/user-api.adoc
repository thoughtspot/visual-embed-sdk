...
[NOTE]
====
* To create a user, you must have the admin user privileges. 
* All users created in ThoughtSpot system are added to ALL_GROUP.
====
...
|`groups` __Optional__|__Array of strings__. A JSON array of GUIDs of groups to assign user to. The user privileges are determined by the group to which you assign the user to. For example, if you assign the user to a group that has the `ADMINISTRATION` privilege, the user will have admin privileges.
|`usertype` __Optional__ a|__String__. Type of user. Default value is `LOCAL_USER`, which indicates that the user is created locally in the ThoughtSpot system.    
|`tenantid` __Optional__|__String__. GUID of the tenant for which the user is being created.
|`visibility` __Optional__|__String__. Visibility of the user. The `visibility` attribute is set to `DEFAULT` when creating a user. The `DEFAULT` attribute makes the user visible for other users and user groups, and allows them to share objects.
|===
...
On succsessful update of the user profile, the API returns the following response code:
...
|`userid` __Optional__|__String__. GUID of the user account to query.
|`name` __Optional__|__String__. User name of the user profile that you want to query.
|===
...
* `visibility`
+
Whether the users of the group can share objects with other users in the group.
...
|`password`|__String__. A new password of the user.
|====
=== Example request
...
|`password`
|__String__. Password of the admin user.
|===
