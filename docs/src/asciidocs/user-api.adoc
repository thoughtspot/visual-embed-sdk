...
If you have a multi-tenant instance, the cluster admin can create a user and associate the user to an `orgid`. For more information about organizations, see xref:orgs.adoc[Multi-tenancy with organizations].
...
|`orgScope` __Optional__|Query parameter|__String__. The Org scope. If you want to add the user to a specific Org, you must set the Org scope to `ALL`.
|===
...
-d 'name=test-user2&password=123%40Cloud&#33;&displayname=test-user2&usertype=LOCAL_USER&visibility=DEFAULT&orgids=%5B%221188864399%22%5D&triggeredbyadmin=false' \
'http://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/?orgScope=ALL'
----

.Request URL
----
https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/?orgScope=ALL
----
...
|`triggeredbyadmin` a|Form parameter|__Boolean__. Indicates if the user update API request is initiated by the ThoughtSpot administrator or an external application. By default, the `triggeredbyadmin` flag is set to `false`. The default value is `false`. +
...
====
|`orgScope` __Optional__|Query|__String__. The Org scope. Specify `ALL` to set the Org scope to all Orgs.
|===
...
'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/{userid}?orgScope=ALL' \
...
|`orgid`|Query parameter|__Integer__. ID of the Org object to which you want to assign the users.
|`orgScope`|Query parameter|__String__. The Org scope. Specify `ALL` to set the Org scope to all Orgs.
|===
...
|`orgScope` __Optional__|__String__. The Org scope. If you want to query user details for all Orgs on a cluster, specify `ALL` to set the Org scope to all Orgs.
|===
...
--header 'Accept: application/json' \
--header 'X-Requested-By: ThoughtSpot' \
'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/?userid=b995e481-d0a7-4820-b1e8-54051ede77a2&orgScope=ALL'
----
.Request URL
...
To remove a user from the ThoughtSpot system and associated org objects, send a `DELETE` request to the `/tspublic/v1/user/{userid}` API endpoint.
...
|`orgid`|Query|__Integer__. The org ID to which the user is associated. This parameter is required only if your ThoughtSpot instance has multi-tenancy enabled. +
...
--header 'Accept: application/json' \
--header 'X-Requested-By: ThoughtSpot' \
'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/ee74b737-da39-4667-95ad-cc06c81ab13d?orgScope=ALL'
----
...
|`principals`|Form parameter|__String__. Specify a list of principal objects in xref:user-api.adoc#principalJson[the JSON file format]. The JSON file can contain all users and groups present in the external database.

|`applyChanges`|Form parameter|
__Boolean__.
Specify whether to sync the users and groups to the system and apply the difference evaluated.
 By default, this attribute is set to `false`.

|`removeDeleted`|Form parameter|
__Boolean__.
...
|`orgScope`|Query|__String__. The Org scope. Specify `ALL` to set the Org scope to all Orgs if you are synchronizing user data at the cluster level.
|===
...
https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/sync?orgScope=ALL
----
