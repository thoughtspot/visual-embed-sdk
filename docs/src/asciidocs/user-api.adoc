...
-d 'username=usr1&url=https%3A%2F%2F<ThoughtSpot-host>.cloud%2F' \ 'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/inactivate'
----
...
Note the user ID and auth token string. You will need this information to reactivate a user account.
...
----
https://sep-eu-central-41.thoughtspotstaging.cloud/callosum/v1/tspublic/v1/user/activate
----
...
To invalidate and revoke a user session, send a `POST` request to the `/tspublic/v1/user/session/invalidate` API endpoint. When you invalidate a user session, the specified users are logged out and are forced to log in again. 

[WARNING]
====
* Use this API with caution as it may invalidate active user sessions and force users to relogin. 
* Make sure you specify the usernames or user IDs. If you pass null values in the API call, all user sessions on your cluster becomes invalid and users are forced to relogin. 
====
