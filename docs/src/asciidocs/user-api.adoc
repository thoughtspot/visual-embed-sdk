...
To activate a user account, send a `POST` request to the `/tspublic/v1/user/activate` API endpoint. 
...
|`auth_token`|__String__. The authentication token to renew user sessions.
|`password` __Optiona__.|__String__. New password of the user account.
...
-d 'userid=fd91f861-3ae6-4bf5-b831-25131b402389&auth_token=gv4HjljdCZKbN4LMEHNn0p7ct12%2F4KSaCwKDfu%2FUVnA%3D' 'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/activate'
----
...
|===
|Form Parameter|Description
|`username`|__String__. Username or the GUID of the user account to deactivate.
...
-d 'username=usr1&url=https%3A%2F%2Fsep-us-west-10.thoughtspotstaging.cloud%2F' \ 'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/inactivate'
----
...
To invalidate and revoke a user session, send a `POST` request to the `/tspublic/v1/user/session/invalidate` API endpoint. When you invalidate a user session, users are logged out and are forced to log in again. 
...
|===
|Form Parameter|Description
|`username`|__Array of strings__. A JSON array of usernames of the users whose sessions you want to revoke.
|`userid`|__Array of strings__. A JSON array of the GUIDs of the user account.
|===
...
--header 'Content-Type: application/x-www-form-urlencoded' \
--header 'Accept: application/json' \
--header 'X-Requested-By: ThoughtSpot' -d 'username=%5B%22tsadmin%22%5D' \ 'https://<ThoughtSpot-host>/callosum/v1/tspublic/v1/user/session/invalidate'
Request URL
----
...
|====
|Form Parameter|Description
|`userid` |__String__. GUID of the user account for which you want to reset the password.
|`auth_token`|__String__. The authentication token used for the user session.
