...
If you have a multi-tenant instance, the cluster admin can create a user and associate the user to a `orgid`. For more information about organizations, see xref:orgs.adoc[Multi-tenancy with organizations].
...
|`orgids`|__Integer__. A JSON array of org IDs to which the user must be added.
|===
...
|`orgid`|__Integer__. The org ID to which the user is associated. This parameter is required only if your ThoughtSpot instance has multi-tenancy enabled.
|===
