...
If the operation is successful, the API returns the following response code:
...
If the password update is successful, the API returns the following response:
...
If the password reset operation is successful, the API returns the following response:
...
If the objects transfer operation is successful, the API returns the following response:
...
|**400**|Invalid `fromName` or `toName`, or no user in the ThoughtSpot system that matches the specified username.
|===
