...
|`orgids` +
__Optional__|__Integer__. A JSON array of org IDs to which you want to add the user. If `orgids` is not defined, ThoughtSpot assigns the user to an Org based on the administrator's session. For example, if the administrator sends the API request from the `Org 0` context, the user is added to `Org 0`.
|===
...
|===
|Form parameter|Description
|`userids`|__String__. A JSON array of user GUIDs to add to the specified org.
|`orgid`|__Integer__. ID of the Org object to which you want to assign the users.
|===
...
|`orgid`|Form data|__Integer__. The org ID to which the user is associated. This parameter is required only if your ThoughtSpot instance has multi-tenancy enabled. +
If you are trying to delete a user that belongs only to the Org specified in the API request, the user is deleted from the Org and ThoughtSpot system. If the user belongs to multiple Orgs and you specify only one Org ID, the user is removed from only from the specified Org and retained in ThoughtSpot. To remove the user from all Orgs and ThoughtSpot system, specify the `orgid` as `-1`.
|===
