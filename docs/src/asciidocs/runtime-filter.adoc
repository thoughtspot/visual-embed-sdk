...
[IMPORTANT]
====
The `DATE` and `DATE_TIME` data type requires you to provide the date and time values in the Epoch time, also known as POSIX or Unix time. The epoch or Unix timestamp is in GMT/UTC time format. You may have to convert time zones to calculate the appropriate timestamp.
