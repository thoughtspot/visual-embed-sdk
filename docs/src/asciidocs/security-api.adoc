...
ThoughtSpot users can get access to objects through the privileges assigned to the groups to which they belong. This type of object permission is referred to as `EFEECTIVE`.
...
To view the details of object permissions for the current logged user, send a `GET` request to the `/tspublic/v1/security/metadata/permissions` endpoint.
...
|`dependentshare` __Optional__|Query parameter|__Boolean__. Object permission details for the depenedent objects. When set to `true`, the API returns the permission details for the depenedent objects for the specified GUIDs.
...
If the `GET` operation is successful, the API returns a response with the permission details for a given object:
...
To view permission details of a specific object for the current logged user, send a `GET` request to the `/tspublic/v1/security/metadata/permissions` endpoint.
...
|`dependentshare` __Optional__|Query parameter|__Boolean__. Object permission details for the depenedent objects. When set to `true`, the API returns the permission details for the depenedent objects for the specified GUID.
|`permissiontype`|Query parameter  a|__String__. Valid values are:
...
For example, to get permission details for specific pinboard and answers, specify the value of `idsbytype` as shown below:
----
{"PINBOARD_ANSWER_BOOK": ["d084c256-e284-4fc4-b80c-111cb606449a"], 	"QUESTION_ANSWER_BOOK": ["92f48c07-9c62-4bfa-81ac-55f3049165b4"]}
----
|`dependentshare` __Optional__|__Boolean__. Object permission details for the depenedent objects. When set to `true`, the API returns the permission details for the depenedent objects for the specified object types and IDs.
|====
