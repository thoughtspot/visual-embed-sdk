...
|`permission`|string a|A string with the GUID of the objects and access privilege. 

* The format of the permission string is `{"permissions":{"<GUID of the object>":{"sharemode":"<type-of-access>"}}}`.
* You can provide `READ-ONLY` or `FULL` access to the objects. With `READ-ONLY` access, the user or user group can view the shared object, whereas `FULL` access enables users to modify the object. 
For example, to provide a read-only access to a pinboard with a GUID of `7a9a6715-e154-431b-baaf-7b58246c13dd`, you can specify the string as `{"permissions":{"7a9a6715-e154-431b-baaf-7b58246c13dd":{"share mode":"READ_ONLY"}}}`

[IMPORTANT]
====
You can also set the `permission` parameter to `NO_ACCESS` to remove a user or user group's access to a shared object.
====
