...
[width="100%" cols="2,1,7"]
[options='header']
|=====
|Operator|Number of values allowed|Example (Visual Embed SDK)
...
==== Apply multiple filters in the SDK
...
liveboardEmbed.render({
    ...
    runtimeFilters: [{
        ...
        {
            columnName: 'state', // eg: color
            operator: RuntimeFilterOp.EQ,
            values: ['California'] // eg: red
        },
        {
            columnName: 'city', // eg: color
            operator: RuntimeFilterOp.EQ,
            values: ['Berkeley'] // eg: red
        }
    ]
});
----
