...
=== REST API v1
The 9.0.0.sw release version introduces the following enhancements:

Session API::
The `/tspublic/v1/session/auth/token` endpoint includes the `autocreate` and `groups` properties to allow administrators to provision a user just-in-time and assign privileges.
