...
Also note the `loadContent()` function referenced in the anchor tag created for the name column in the function above is is a placeholder representing whatever is necessary to load taht type of ThoughtSpot content in the web application. Loading content is discussed in the next section.
