...
The group APIs allow administrators to programmatically assign the following types of privileges to a user group:
...
[NOTE]
====
The API removes only the user association from a user group. It does not delete the user from the Thoughtspot system. To delete the user, use the xref:user-api.adoc#delete-user[*DELETE* /tspublic/v1/user/{userid}] API.
====
