...
The group APIs allow adminstrators to programmatically assign the following types of privileges to a user group:
...
* `DATAMANAGEMENT` 
+
Allows users to create worksheets and views. To edit a worksheet or view created and shared by another user, the user must have the edit permission to modify the object.

* `SHAREWITHALL`
+
Allows users to view the names and share objects with users of another group. 

* `EXPERIMENTALFEATUREPRIVILEGE`
+
Allows access to trial and experimental features that ThoughtSpot makes available to evaluating users and early adopters.
...
* `BYPASSRLS`
+
Allows users to perform the followig functions:  
...
If you have admin user privileges, you can programmatically modify the properties a group using the `/tspublic/v1/group/{groupid}` API. Using this API, you can assign privileges and modify the group visibility setting.
...
On succsessful update of the user group profile, the API returns the following response code:
...
To programmatically remove a user from user group, send a `DELETE` request to the  `/tspublic/v1/group/{groupid}/user/{userid}` API endpoint.

[NOTE]
====
The API removes only user association from a user group. It does not delete the user from the Thoughtspot system. To delete the user, use the xref:user-api.adoc#delete-user[*DELETE* /tspublic/v1/user/{userid}] API.
====
...
|===
|Path Parameter|Description
|`groupid`|__String__. The GUID of the user group tfrom which you want to remove the user.
