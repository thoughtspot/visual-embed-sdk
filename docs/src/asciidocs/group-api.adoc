...
If you have a multi-tenant instance, the cluster admin can associate a group to an `orgid`. For more information about organizations, see xref:orgs.adoc[Multi-tenancy with organizations].
...
|`orgid` |__Integer__. ID of the organization to which you want to map the group. You can define this attribute if ThoughtSpot is set as a multi-tenant instance and is logically partitioned into organizations.
|===
