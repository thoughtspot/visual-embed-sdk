...
[NOTE]
====
You can use either `groupid` and `name` to query data for a specific. If using both, make sure the values for these parameters point to the same user account. If neither of these parameters is defined, the API returns a response with the details of all user groups in the ThoughtSpot system. 
====
