...
The following table lists the authentication methods supported on an embedded ThoughtSpot instance and general recommendations for choosing an authentication method for your app.
...
|xref:embed-authentication.adoc#passthru[Passthrough authentication]|`AuthType.Passthrough` +
`AuthType.None` a| Use this method: +

* If your application is already using an Identity Provider (IdP), and you have access to your IdP settings to provision users. +
* If your application is using a SAML or OIDC IdP that supports embedded applications and allows iframe redirects
* If you want to embed ThoughtSpot in your app without using the Visual Embed SDK.
a|ThoughtSpot does not recommend using pass-through authentication in the following scenarios: +

* If you are using multiple IdPs +
* If you cannot configure your IdP to support embedded authentication +
* If you want to customize the authentication flow; for example, automatically log in a user when their session expires.
|xref:embed-authentication.adoc#saml-sso-embed[SAML SSO authentication]|`AuthType.SAML` a|Use this method: +

* If you have an IdP, but it does not natively support embedding
* If you have access to your IdP settings to allow iframe embedding
a |ThoughtSpot does not recommend this method: +

* If you want to avoid redirecting your application users to authenticate to the IdP. +
This method also requires additional configuration, such as setting `noRedirect`  and redirect path parameters in the SDK.
* If you are embedding ThoughtSpot without using the Visual Embed SDK
|xref:embed-authentication.adoc#oidc-auth[OpenID Connect authentication]|`AuthType.OIDC` a|Use this method: +

* If you have an OpenId provider, but it does not natively support embedding.
* You do not have access to your OpenID provider server to configure it for pass-through authentication.
a|ThoughtSpot does not recommend this method: +

* If you want to avoid redirecting your application users to authenticate to the OpenID provider. +
This method also requires additional configuration, such as setting redirect path parameters in the SDK.
* If you are embedding ThoughtSpot without using the Visual Embed SDK.
|xref:embed-authentication.adoc#basic-auth-embed[Basic authentication]|`AuthType.Basic` a|Use this method: +

* If you want to local authentication with `username` and `password`.
...
|xref:embed-authentication.adoc#trusted-auth-embed[Trusted authentication]|`AuthType.AuthServer` a|Use this method: +

* If you want to customize the authentication flow; For example, setting up automatic login.
* If your IdP setup does not support pass-through authentication.
* If you want service users that do not exist in your IdP to authenticate to ThoughtSpot. a|ThoughtSpot does not recommend using trusted authentication in the following scenarios: +

* If you want to implement a simple solution with pass-through authentication.
* If you don't have the resources required to implement trusted authentication and customize authentication flow.
|=====


[IMPORTANT]
====
For production environments, ThoughtSpot strongly recommends using SAML SSO, OIDC, pass-through, or trusted authentication.
====
...
ThoughtSpot recommends pass-through authentication, because it simplifies the authentication process for embedded applications. With pass-through authentication, you can leverage your existing IdP setup and configure it to support your embedded instance. All you need to do is to configure SAML or OIDC authentication support on ThoughtSpot and set up redirects to your IdP for seamless user authentication.
...
[Note]
====
The pass-through authentication feature is similar to the `No authentication` method supported in the SDK. If you have set `authType` as `AuthType.None`, the SDK considers it as pass-through authentication and allows your embedded application to use your current IdP configuration and redirect rules.
====
...
* `**authEndpoint**`
+
Specify the authentication endpoint URL from which you want to obtain the authentication token. When an API request for is sent to the specified endpoint URL, the authentication server sends the token as plain text.
