...
In the trusted authentication method, a security token is required to authenticate users who request access to the embedded ThoughtSpot content. For token-based authentication, you will require an authenticator server or service, which can securely authenticate users your application users. 

The following attributes are required for obtaining a login token for an embedded application user:
...
[NOTE]
====
* The generated authentication token is valid for only five minutes.  
* Make sure the `getAuthToken` function returns a fresh token for each re-login.
====
