...
[IMPORTANT]
====
To obtain a token and log in the user, the user must already be created in ThoughtSpot. For just-in-time (JIT) provisioning of a user and groups assignment, use the xref:session-api.adoc#session-authToken[/tspublic/v1/session/auth/token] API endpoint.
====
