...
Runtime filters can be updated after load time by triggering the `HostEvent.UpdateRuntimeFilters` event (documented in reference below). You can build the filter UI in the embedding app, which triggers the `UpdateRuntimeFilters` event when changed or on the click of an `apply` button.
