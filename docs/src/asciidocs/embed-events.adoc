...
You define a *listener* for an EmbedEvent with a callback function to handle the event response via the `.on(embedEvent, eventHandlerCallbackFunction)` format.
...
xref:runtime-filters.adoc[Runtime filters] can be set programmatically before loading the embedded ThoughtSpot content in the `options` object set in the `LiveboardEmbed` component contructor. 

Runtime filters can be updated after load time by triggering the `HostEvent.UpdateRuntimeFilters` event (documented in reference below). You can build filter UI in the embedding app UI which triggers the `UpdateRuntimeFilters` event when changed or when an "apply" button is pressed.

==== Filtering from selection
Filtering from selection on a chart or table can be implemented by combining the `EmbedEvent.VizPointClick` or `EmbedEvent.VizPointDoubleClick` events with the `HostEvent.UpdateRuntimeFilters` event. 

The callback function from the `VizPointClick` event will need to read the response, parse out the attribute(s) from the response that will be sent to the Runtime Filters, then send package the attributes and their target fields in the format used by `UpdateRuntimeFilters`.
...
The `hideResults` parameter in the options object of a SearchEmbed constructor blocks the *GO* button from displaying the chart or table results. When this option is *true*, you can listen to the `QueryChanged` event among others to perform actions based on the user's interaction within the SearchEmbed component.
