...
[NOTE]
====
When you assign a custom action to a worksheet, ThoughtSpot adds it only to the new answers generated from a worksheet. The custom action will not be available for the existing saved answers that were generated using the worksheet.
====
