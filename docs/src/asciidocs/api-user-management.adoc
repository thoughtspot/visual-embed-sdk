...
To provide access to ThoughtSpot content or define role-based privileges, you need to add users and groups in ThoughtSpot. You can add link:https://cloud-docs.thoughtspot.com/admin/users-groups/add-user.html[users,  window=_blank] and link:https://cloud-docs.thoughtspot.com/admin/users-groups/add-group.html[user groups]  either manually in the UI or by using the xref:rest-api-reference.adoc[REST API services].
