...
. If you plan to add routes, install routing support for your app and update the `index.jsx` (for React app using Javascript) or `index.tsx` (for React app using TypeScript) file.

+
----
ts-data-app> npm add react-router-dom@6
----
...
===  Import SearchEmbed React component

. Add the following imports to `App.jsx` (for React app using Javascript) or `App.tsx` (for React app using TypeScript):
...
// the React components as shown here:
import { SearchEmbed } from '@thoughtspot/visual-embed-sdk/lib/src/react';
----
. Initialize the connection within the `App()` function in the `App.jsx` (for React app using Javascript) or `App.tsx` (for React app using TypeScript) file, and specify the authentication method. 
+
...
. In the search page, import the React library: 
+ 
[source.Typescript]
----
...
== Embed a Liveboard or visualization

To embed a ThoughtSpot Liveboard or visualization on a Liveboard, complete the following steps:

===  Import LiveboardEmbed React component

. Add the following imports to `App.jsx` (for React app using Javascript)  or `App.tsx` (for Typescript app):
...
For more information about other authentication methods and login attributes, see xref:embed-authentication.adoc[Authentication].

=== Create a Liveboard page (For Liveboard embedding)
...
. In the Liveboard page, import the React library: 
+ 
[source.Typescript]
----
import { Action, EmbedEvent, HostEvent } from "@thoughtspot/visual-embed-sdk";
...
=== Create a visualization page (For visualization embed)
...
. In the visualization page, import the React library: 
+ 
[source.Typescript]
----
import { Action, EmbedEvent, HostEvent } from "@thoughtspot/visual-embed-sdk";
...
* Verify if embedded visualizations are rendered.
...
=== Import AppEmbed React component
...
. In the `fullApp` page, import the React library: 
+
[source,Typescript]
----
import {
