...
Before you begin embedding ThoughtSpot, make sure you have installed React framework and its dependencies on your setup, and create a React app to embed ThoughtSpot components.

. Check if link:https://docs.npmjs.com/downloading-and-installing-node-js-and-npm[NPM and Node.js are installed] in your setup.
...
----
+
For information about how to install React framework on Windows, see  link:https://docs.microsoft.com/en-us/windows/dev-environment/javascript/react-on-windows[Install React directly on Windows, windows=_blank].
...
To add HTML code, rename these files to `.jsx`. 
If you are building an app using TypeScript, you can rename the files to `.tsx`. +
...
=== Check the localhost port setting

By default, React uses Port 3000 as a localhost port. Make sure you link:security-settings.adoc#csp-viz-embed-hosts[add `localhost:3000` as a CSP visual embed host] in the **Security Settings **page of the **Develop **tab.

If you want to use Port 8000 instead, make sure you link:security-settings.adoc#csp-viz-embed-hosts[add it as a CSP visual embed host] and update the following script in the `package.json` file in your app directory.
...
[#sdk-init]
=== Initialize the SDK and set authentication type

To initialize the SDK, you must point the SDK to your ThoughtSpot host URL. In the `init` call, you must specify the authentication method.
...
In the following code sample embeds search with the data source pane, disables UI actions such as *Pin* and **Show underlying data**, registers `EmbedEvent.Load`, `HostEvent.Search`, `EmbedEvent.QueryChanged` events, and logs these events in the console.
...
The following code sample embeds a Liveboard, disables UI actions such as *Share* and **Delete**, sets specific visualization GUIDs as visible visualizations, and registers the `EmbedEvent.Load`, `HostEvent.SetVisibleVizs`, and   `HostEvent.UpdateRuntimeFilters` events. 

The code sample also includes the `applyFilter` function, which can be called on the click on a button or a data point on a visualization to trigger `HostEvent.UpdateRuntimeFilters` event and update runtime filters.  
...
* An `updateFilter` function to trigger `HostEvent.UpdateRuntimeFilters` event and update runtime filters, when the user double-clicks a data point on the visualization. 
...
The example also shows how to register a custom action event to trigger data payload from a saved answer when a user clicks the `Send data` custom action. For more information about custom action events, see xref:custom-actions-callback.adoc[Callback custom actions].
