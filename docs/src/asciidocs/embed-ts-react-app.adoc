...
* The `searchOptions` prop to programmatically construct a query string with `[quantity purchased] [region]` keywords and execute the search query.  
...
In the following code sample embeds search with the data source pane, disables UI actions such as *Pin* and **Show underlying data**, registers event handlers for `Load`  `QueryChanged`.
