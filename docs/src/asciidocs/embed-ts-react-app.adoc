...
* A `Search` function with a data source ID and the `[quantity purchased] [region]` search token string.
* A `changeSearch` function to allow programmatic change of the search query. You can assign this function to a button or custom action.
* The `HostEvent.Search` event to trigger a new search when the query changes to `[sales] by [item type]`.
+
[source,Typescript]
----
const Search = () => {
