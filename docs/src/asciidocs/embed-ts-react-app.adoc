...
The example also shows how to register a custom action event and trigger a data payload from a saved answer when a user clicks the `Send data` custom action. For more information about custom action events, see xref:custom-actions-callback.adoc[Callback custom actions].
