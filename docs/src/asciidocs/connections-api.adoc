...
|`metadata` a|__String__. A JSON map of the connection metadata. The metadata must include xref:connections-api.adoc#connection-attr[configuration attributes], database, schema, and table objects. Make sure the database object in the metadata JSON includes at least one table.
|`state`|_Integer_. The GUID of the OAuth redirect URI. This attribute is automatically populated and cannot be set by the user.
|====
...
The following example shows the metadata JSON map for updating a Snowflake data connection. Note that the example shows the attributes required to authenticate to the data source and the object metadata (tables or columns) to modify. 
