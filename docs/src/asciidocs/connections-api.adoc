...
The  metadata JSON includes configuration attributes and a map of the database schema with tables. The configuration attributes vary based on the type of connection. To create a new data connection, the metadata JSON map should include at least one table object. 

[div boxDiv boxFullWidth]
--
.Snowflake connection metadata 
...
* `role`
+
_String_. Role or the group privilege assigned to your user account in ThoughtSpot. For example, `sysadmin`.
...
[div boxDiv boxFullWidth]
--
.Amazon Redshift connection metadata
...
--
.Google BigQuery connection metadata
...
--
.Azure Synapse connection metadata
...
--
.Oracle ADW connection metadata
...
--
.Databricks connection metadata
...
|`metadata` a|__String__. A JSON map of the connection metadata. The metadata must include xref:connections-api.adoc#connection-attr[connection attributes], database, schema, and table objects. 
