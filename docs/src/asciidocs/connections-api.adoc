...
ThoughtSpot supports connecting to external data warehouses and using these as data sources for analytics and visualizations. To create a connection to an external data sources and set up a live query service, use the `/tspublic/v1/connection/create` API. 
...
To export metada of an active connection map, send a `GET` request to the `/tspublic/v1/connection/export` API. 
...
[IMPORTANT]
====
A connection can be used in multiple data sources or visualizations. If the data connection has dependent objects, make sure you remove its associations before deleting. 
====
