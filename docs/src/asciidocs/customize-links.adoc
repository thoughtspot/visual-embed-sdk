...
Generic link::
+
The *Generic* link allows you to customize the URLs of ThoughtSpot application pages and navigation links that are not parameterized. For example, the top navigation menu links in your embedded application can be customized as *Generic link*.
...
Generic link::
+
The *Generic link* setting is applied to the URLs of ThoughtSpot application pages and navigation links that are not parameterized using the other link setting options in the *Customization* > *Link settings* page.
...
+
You must also set the `linkOverride` to `true` to override the link format of your embedded application pages and navigation links:
