...
For example, when a user subscribes to monitor alerts, ThoughtSpot sends email notifications with a link to the KPI chart along with an *Unsubscribe* link to allow the user to unsubscribe from the alert at any time. When you include {ts-query-params} in the link string, ThoughtSpot passes the query parameters required for unsubscribing a user from a monitor alert. +
