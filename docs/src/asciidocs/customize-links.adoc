...
For example, when a user subscribes to receive monitor alerts for a KPI chart, ThoughtSpot sends email notifications with a link to the KPI chart along with an *Unsubscribe* link to let the user unsubscribe from alerts at any time. When you include `{ts-query-params}` in the link string, ThoughtSpot passes the query parameters required for unsubscribing a user from a monitor alert. +
