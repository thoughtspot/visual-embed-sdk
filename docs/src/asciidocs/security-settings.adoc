...
The following table shows the allowed domain name strings to use in the CORS and CSP allowlists. 
...
3+| Allowed in the UI. However, we recommend that you do not specify the protocol in the domain URL string for CSP and CORS. 
...
See the xref:security-settings.adoc#csp-cors-hosts[note about excluding protocol].
|[tag greenBackground]#✓# Supported|[tag greenBackground]#✓# Supported|[tag greenBackground]#✓# Supported
