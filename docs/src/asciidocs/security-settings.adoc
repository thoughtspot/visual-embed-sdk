...
* Although the UI allows a domain URL with the protocol (`http/https`) in the CSP allowlist, we recommend that you do not include the protocol in the domain name string.
...
The following table shows the allowed domain name strings in the CORS and CSP allowlists. 
...
////
Domain URL strings with protocol

* `\http://thoughtspot.com` +
* `\https://thoughtspot.com` +
* `\https://www.thoughtspot.com` +

Allowed in the UI. However, we recommend that you do not specify the protocol in the domain URL string for CSP and CORS. 
////
|||
...
See the xref:security-settings.adoc#csp-cors-hosts[note about excluding protocol in CSP allowlists].
