...
* If you have embedded ThoughtSpot content within your application page in an inline frame, web browsers may block the  embedded content. If your host application and embedded ThoughtSpot instance are on different domains, some web browsers block cross-domain requests by default. To enable seamless interaction between your host application and the embedded ThoughtSpot instance, you need to configure the following security settings in ThoughtSpot:
 
** Trusted hosts for CORS
+
To allow your application to call ThoughtSpot, access its resources, and render the embedded content, you must add your domain as a trusted host and enable Cross-Origin Resource Sharing (CORS). 

** Trusted hosts for CSP
+
To allow your application to embed ThoughtSpot and access its content via web browsers, add your application domain as a trusted host in the CSP allowed list. You need to also add the domain URLs to the `connect-src` allowed list to load script interfaces and Javascript events.

+
Users with admin or developer privileges can configure the CORS and CSP allowed list on the *Security Settings* page in the *Develop* tab of the ThoughtSpot UI.
...
[#csp-connect-src]
== Add URLs to CSP connect-src list
...
The CORS configuration for your cluster controls which domains can access and modify your embedded content. To allow your application to call ThoughtSpot or its REST API endpoints, and request resources, you must add your application domain to the CORS allowlist. For example, if your website is hosted on the `example.com` domain and the embedded ThoughtSpot content is hosted on the `example.thoughtspot.com`, you must add the `example.com` domain to the CORS allowlist for cross-domain communication. 

After you enable CORS for your application domain, ThoughtSpot adds the `Access-Control-Allow-Origin` header in its API responses when your host application sends an API request to ThoughtSpot. 

You can add `\http://localhost:8080` to test your deployments locally. However, we recommend you disable the `localhost` access in production environments.

To add a domain to the CORS allowlist, follow these steps:
...
The following table shows the valid domain name strings to use in the CORS and CSP allowed list. Note that the CORS allowed list ignores the port and protocol in the hostname strings. 

[width="100%" cols="4,2,2,2"]
[options='header']
|====
|Domain name format|CSP Visual Embed host|CSP connect-src|CORS

a|Domain URL strings for localhost with or without port and protocol.  
...
a|Domain URL strings with or without the protocol. 
