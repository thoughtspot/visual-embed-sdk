...
When embedding ThoughtSpot, if you have xref:getting-started.adoc#_block_access_to_non_embedded_thoughtspot_instance[set the `blockNonEmbedFullAppAccess` property to `true` in Visual Embed SDK], your application users cannot access or navigate to the ThoughtSpot application experience outside the context of your app.

However, users with administration or developer privilege can access ThoughtSpot in the non-embed mode. They can also enable or turn off the *Block non-embed full app access* feature on the *Develop* > *Customizations* > *Security Settings* page. The `blockNonEmbedFullAppAccess` setting in the SDK takes precedence over the *Block non-embed full app access* setting on the *Security Settings* page.
