...
To create and manage an Org on a multi-tenant ThoughtSpot instance, you must have Cluster administrator privileges. By default, the administrator of the `Primary Org (Org 0)` is set as a  cluster administrator.

To perform CRUD operations on Org objects, the cluster administrator must be in the `All Org` context. To set the `All Org` scope, specify the `orgId` value as `-1`.

== Supported _operations_
...
|`orgId`|Query|__Integer__. ID of the Org context. Set the `orgId` value to `-1`.
...
|`orgId`|__Integer__. The ID of the Org context. Set the `orgId` value to `-1`.
|===
