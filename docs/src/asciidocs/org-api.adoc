...
To perform CRUD operations on Orgs, the cluster administrator must use the `All Org` scope by setting the `orgId` value as `-1` in API requests.
...
|`orgId`|Query|__Integer__. ID of the Org context. To set the scope to `ALL ORGS`, specify `-1`.
...
|`orgId`|__Integer__. ID of the Org context. To set the scope to `ALL ORGS`, specify `-1`.
|===
