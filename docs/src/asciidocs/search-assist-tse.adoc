...
The Search Assist feature provides a walkthrough of sample scenarios of searching data from a worksheet. Your application users can use this feature to familiarize themselves with the search experience and get the answers they need.

To enable Search Assist walkthrough on an embedded instance, the following steps are required:

* Enable Search Assist in the Visual Embed SDK (__Requires Developer privilege__)
...
To enable Search Assist, set the `enableSearchAssist` parameter to `true` as shown in this example.
...
By default, the Sample Retail link:https://docs.thoughtspot.com/cloud/latest/system-worksheet[system worksheet, window=_blank] includes a Search Assist lesson with a pre-defined set of questions and instructions to guide your application users.
...
For example, if you are searching data from the Sample Retail worksheet, the initial example asks *_What were Sales by Product in this year?_* and Search Assist guides you to search for *_sales_* and press *Enter* on your keyboard. The search then returns the Answer as a table, demonstrating your total sales.
...
* Search Assist is supported only if ThoughtSpot Search is embedded using the `SearchEmbed` SDK library.
* You can configure a Search Assist lesson only if you have embedded the *Data* tab in your host app.
