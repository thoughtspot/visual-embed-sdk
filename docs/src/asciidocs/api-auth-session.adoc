...
Note that if you are using the Web browser or Postman to make a REST API call, the session cookie obtained from the  `/tspublic/v1/session/login/token` API call is automatically set. REST clients in a non-browser environment must pass the session cookie in the request header as shown in the following example:
