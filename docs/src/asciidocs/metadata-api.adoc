...
====
|`subtypes` __Optional__ a|__String__. A JSON array specifying the sub-types of a metadata object. 

* For the `LOGICAL_TABLE` metadata object `type`, you can query data objects of specific subtypes.
...
====
* For `LOGICAL_COLUMN` and `LOGICAL_RELATIONSHIP` metadata object types, you can query data for any of the following `subtypes`:
** `FORMULA` to query a list of formulas applied to a worksheet.
