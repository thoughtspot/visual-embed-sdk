...
* To set an ascending order, specify `true`.
* To set a descending order, specify `false`.
|`offset` __Optional__|__Integer__. The batch offset value that indicates the index point starting from which the list of object headers should be retrieved. The system default is -1, which implies the first item in the list. fetch the page headers. The system default is  `-1`, which  implies first page.
