...
If no branch is specified, the branch name specified in the `default_branch_name` for this configuration will be considered. ThoughtSpot recommends maintaining GUID mapping file in a separate branch.
|===
...
Starting with the 9.4.0.cl release, the version control APIs can automatically generate a GUID mapping file for your deployment instance and save this file in a Git branch. The mapping file records the GUIDs for each TML object as shown in this example:
...
Building a release version for a `Prod` environment on the same instance requires swapping in the correct GUIDs. If you enable xref:_guid_mapping[GUID mapping] in the Git configuration, the version control APIs will automatically generate a GUID mapping file and update object references when deploying your commits to the destination environment.
