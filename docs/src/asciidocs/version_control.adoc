...
5. xref:version_control.adoc[Deploy commits from the `main` branch to your `Prod` environment] and deploy your content.
...
|`enable_guid_mapping`  +
__Optional__ |__Boolean__. When set to `true`, ThoughtSpot maps the GUID of the TML objects to the deployment instance and generates a GUID mapping file in the specified Git branch. For more information, see xref:version_control.adoc#_guid_mapping[GUID mapping].
...
If no branch is specified, the branch name specified in the `default_branch_name` for this configuration will be considered.
|===
...
curl -X POST \
  ...
  "guid_mapping_branch_name": "guid-mapping"
}'
----
...
Every object in ThoughtSpot is assigned a unique *GUID* as a reference. When deploying TML files, if the TML representation does not have the same unique GUIDs for objects on the source and destination instances, it's essential to map the newly created object GUIDs on the destination environment with the GUID of the objects imported from the source cluster. You can track the GUIDs for TML objects in a mapping file and swap these out within the TML files to make your deployment work seamlessly.

Starting with the 9.4.0.cl release, you can generate a GUID mapping file using Version Control API on your deployment instance and save this mapping file in a Git branch. The mapping file records the GUIDs for each TML object as shown in this example:
...
* `counter` shows the number of times the mapped object is updated.

When xref:version_control.adoc#_deploy_commits[publishing content to another environment], make sure to swap the GUIDs from the source environment with those of the equivalent objects in the destination environment within the TML files. If you are deploying from a `dev` cluster to a `prod` instance and if the GUIDs are being re-used across environments, GUID mapping is not required.
