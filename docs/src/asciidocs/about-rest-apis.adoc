...
* REST API V2 [tag orangeBackground]^BETA^
+
The REST API V2 framework is a new developer solution that provides additional functionality and interactive experience for developers. In the REST API V2 version, ThoughtSpot offers SDK and language-specific client libraries. 


api/rest/playgroundV1
...
== REST API Playground [tag orangeBackground]

Starting from the 8.0.0.cl release, ThoughtSpot users with Developer or Administrator privileges can access the REST v1 and v2 endpoints in the Developer portal. If the REST API SDK and Playground feature is enabled on your cluster, the REST Playground v1 and REST Plaground v2 menu options will be available under the *REST API* category in the *Develop* tab.
...
* To access the xref:rest-api-v1.adoc[REST v2 SDK and Playground], click **REST Playground v2**.
...
* xref:rest-apiv2-reference.adoc[REST API v1 Reference]
