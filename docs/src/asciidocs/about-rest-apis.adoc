...
* REST API V2 
+
The REST API v2 [beta orangeBackground]^BETA^ framework is a new developer solution that provides additional functionality and interactive experience for developers. In the REST API V2 version, ThoughtSpot offers SDK and language-specific client libraries.
...
Starting from the 8.0.0.cl release, ThoughtSpot users with Developer or Administrator privileges can access the REST v1 and v2 endpoints in the Developer portal. If the REST API SDK and Playground[beta orangeBackground]^BETA^ feature is enabled on your cluster, the REST Playground v1 and REST Plaground v2 menu options will be available under the *REST API* menu in the *Develop* tab.
