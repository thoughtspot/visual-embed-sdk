...
When you embed ThoughtSpot into your app, your application URL will be set as  `<your-company-name>.thoughtspot.cloud` by default. The system-generated email notifications from your embedded ThoughtSpot instances will also include `thoughtspot.cloud` in the domain name.

For branding consistency and security purposes, you might want to use your own domain name. For example, you might want to use `<app-name>.<your-company-name>.com` as your base URL, instead of `<your-company-name>.thoughtspot.cloud`. 
...
For all such domain customizations, ThoughtSpot recommends that you contact the ThoughtSpot Support team. We recommend that you read the following guidelines  before initiating a request for the URL or domain name change.
...
|Applicable only if Amazon Simple Email Service is used for emails.
