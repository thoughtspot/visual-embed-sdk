...
The API Plaground consists of following areas:
...
+
The search bar in the resource pane allows you to search for endpoint and navigate between different endpoints in the list.

API request editor::

The middle section of the portal provides a short description of the endpoint, endpoint URL, request parameters and response codes. If a request parameter requires the input format as a JSON array, you can add each array separately. For example, if you are assigning privileges to a group, you can add the privilege strings as an array.   
...
* Copy  icon
+
Allows copying API requests and responses.
...
* Download icon
+
Downloads the API response JSON file to your local directory.
...
To download the SDK library for a specific language, select the language and click **Get SDK**. You can also download the SDK and API libraries from the link:https://github.com/thoughtspot/visual-embed-sdk[GitHub repository, window=_blank]. 

For information see, xref:rest-api-sdk-libraries.adoc[REST API SDK and client libraries].
