...
ThoughtSpot categorizes the objects related to a worksheet, table, or column as dependents. Before editing or deleting a data objects such as worksheets, tables, and columns, you must verify the dependent objects, and add or remove associations if required. To query the details of dependent objects and associate objects as dependents, you can use the `tspublic/v1/dependency` APIs.
...
* the `Accept` header set as `Accept: application/json`
...
|`type` a|__String__. Type of the data object. Valid values are:
...
To get a list of objects with incomplete metadata, send s `GET` request to the `GET /tspublic/v1/dependency/listincomplete` API endpoint.
