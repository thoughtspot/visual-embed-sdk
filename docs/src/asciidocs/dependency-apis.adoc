...
* `PINBOARD_ANSWER_BOOK` for Liveboard visualizations.
...
If a Liveboard is set as a default Liveboard for a user group, the `/tspublic/v1/dependency/pinboard` endpoint will return the  user group as a dependent object for that Liveboard. The API request does not return data objects such as tables, views, and worksheets as dependent objects.
...
|====
|Query Parameter|Description
|`id`|__Array of strings__. A JSON array of the GUIDs of the Liveboard.
|====
