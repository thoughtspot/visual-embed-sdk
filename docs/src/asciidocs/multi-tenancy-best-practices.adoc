...
ThoughtSpot recommends using Orgs [beta betaBackground]^Beta^ for multi-tenancy. If you do not have this feature enabled on your instance and do not want to set up Orgs [beta betaBackground]^Beta^, you can use the following options:
...
==== Content belonging to individual tenants
To allow users to create their own content and share only within their organization, you will create at least one group for each tenant, separate from the application tenant group. This group can be set to *SHARABLE*, or you may want additional groups below the main tenant group, representing different sets of users who belong to that tenant, and then make those child groups the ones that are *SHARABLE*.
