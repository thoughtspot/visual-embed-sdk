...
* The `**POST** /api/rest/2.0/users/{user_identifier}/update` and `**POST** /api/rest/2.0/groups/{group_identifier}/update` support specifing the operation type in the API request. For example, if you are removing a property of a user or group object, you can specify the `operation` type as `REMOVE`.
...
The following Version Control [beta betaBackground]^Beta^ API endpoints are now available for lifecycle management of content on your deployment environments:
