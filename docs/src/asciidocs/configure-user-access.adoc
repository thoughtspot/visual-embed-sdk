...
Using RLS, you can restrict access to sensitive data and filter tables to show only relevant data for the signed-in user. *RLS rules* are applied at the table level, so it automatically extends to all worksheets, saved answers, and Liveboards based on that table, every time.

+
Administrators and table owners define RLS rules on Thoughtspot table objects which use either the signed-in user's *username* or the set of the ThoughtSpot *group names* of which the user belongs to. The RLS rules translate into WHERE clauses in any query using the table object. 

+
RLS rules on one table object can reference other tables objects that are joined within a given connection. 
