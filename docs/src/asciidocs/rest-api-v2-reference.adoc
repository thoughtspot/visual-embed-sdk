...
[NOTE]
====
Access to ThoughtSpot data via API is restricted based on xref:api-user-management.adoc#group-privileges[user privileges] and xref:configure-user-access.adoc#_object_level_permissions[object-level permissions]. Some operations are available for all users, but require at least view access to the metadata object. ThoughtSpot users are assigned access privileges based on the group to which they belong. Additionally, metadata owners can restrict access to the object by assigning view-only or edit permissions when sharing an object with other users and groups.
====
...
__Requires administrator privilege or edit access to the metadata object (Liveboard, saved Answer, or the Worksheet)__.
