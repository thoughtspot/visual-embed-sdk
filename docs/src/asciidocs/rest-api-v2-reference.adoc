...
You can also filter the API output based on groups, visibility, account status, user type, and user preference settings.
...
The Orgs feature is supported only on ThoughtSpot application instances hosted on Cloud.
...
Gets the details of group objects from ThoughtSpot. You can get the details of a specific group or all groups available in the ThoughtSpot system. You can also filter the API output based on user association, privileges, visibility, and group type.
