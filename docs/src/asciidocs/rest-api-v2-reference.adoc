...
This API Reference page lists the REST API v2 endpoints and provides basic information about the resource URLs and supported operations. If you want to try out these APIs, view detailed documentation, and download code samples, visit the +++ <a href="{{previewPrefix}}/api/rest/playgroundV2" target="_blank">REST API v2 Playground</a>+++.
...
Allows a user to log in to ThoughtSpot with username and password. If the login is successful, a user session is established.
--

[div divider]
--
+++<h4>Get Object Access Token</h4>+++

`POST /api/rest/2.0/auth/token/object` +

Allows obtaining an authentication token from ThoughtSpot to access a specific metadata object. The token obtained from this API call does not provide full access to ThoughtSpot.

* To get a bearer token, specify the username, password, and object ID in the API request.
* To get a token for trusted authentication, specify the username, object ID, orgId, and `secret_key`.

+
If the user object is not created in ThoughtSpot, you can select `autocreate` to provision the user and assign groups and privileges.
+
The ThoughtSpot generates a `secret key` if Trusted authentication is enabled on your instance. You can copy the `secret key` from *Develop* > *Customizations* > *Security Settings* page.
--

[div divider]
--
+++<h4>Get Full Access Token</h4>+++

`POST /api/rest/2.0/auth/token/full` +

Gets an authentication token from ThoughtSpot for a user. The token obtained from this API call provides full access to ThoughtSpot. +

* To get a bearer token, specify the username and password in the API request.
* To get a token for trusted authentication, specify the username, orgId, and `secret_key`.
+
The ThoughtSpot generates a `secret key` if Trusted authentication is enabled on your instance. You can copy the `secret key` from the *Develop* > *Customizations* > *Security Settings* page.
+
If the user object is not created in ThoughtSpot, you can select `autocreate` to provision the user and assign groups and privileges.
--
...
Revokes authentication token for a user.
--
...
Gets user details. You can get the details of a specific user or all user objects available in the ThoughtSpot system. You can specify additional parameters to filter API response.
--
...
This API also supports the following operations:

* add the email address of the user
* add the user to Orgs and groups.
* set account status
* define sharing visibility
* set a default Liveboard for the user
* add to the user's favourites list
* set user preferences to start or stop onboarding walkthrough, and receive email notification when another user shares an object
--
...
* If the specified user is not available in ThoughtSpot, users are created and assigned a default password. The `default_password` parameter is optional.
* If the `delete_unspecified_users` property is set to `true`, ThoughtSpot users not specified in the API request, except for `tsadmin`, `guest` system and su users, are deleted.
* If the specified user objects are already available in ThoughtSpot, the object properties are modified in ThoughtSpot and synchronized as per the data defined in the API request.
