...
+
Gets an authentication token from ThoughtSpot, which provides full access to ThoughtSpot application.
+
...
* add Liveboard, Answer, and Worksheet objects to the user's favourites list
...
* If the `delete_unspecified_users` property is set to `true`, users not specified in the API request, except for `tsadmin`, `guest` system and su users, are deleted.
...
Allows changing a given user's password.
...
[NOTE]
====
* In the current release, REST API v2 Playground is accessible only in the primary Org (Org 0) context. However, REST clients can access the REST v2 endpoints via a Web browser, Postman or other such tools.
...
* If the `delete_unspecified_groups` property is set to `true`, the groups not specified in the API request, except `administrator`, are deleted.
...
__Requires at least view access to metadata objects__.
...
[NOTE]
====
Searching by metadata sub-types such as `CALENDAR_TYPE` and `FORMULA` is not supported in REST API v2.
====
...
__Requires at least view access to the data source__.
