...
The following example shows how to register the `RouteChange`, which is triggered when a user navigates to another page within the embedded ThoughtSpot interface.

[source,JAVASCRIPT]
----
appEmbed.on(EmbedEvent.RouteChange, (data) => {
...
export const getVisualizationList = async (pinboardId) => {
    // Returns a list of visualizations pinned on a pinboard
