...
In this method, the REST clients must xref:authentication.adoc#trustedAuthToken[obtain a login token] to sign in to a ThoughtSpot instance. Before sending an API request to get login token from ThoughtSpot, you must enable *Trusted authentication* on the *Security Settings* page in the *Develop* tab and get the `secret_key`. You must send the  `secret_key` in your API request to get a login token from ThoughtSpot. After you receive the token, make sure you send it in the `Authorization header` in your API calls. +


Cookie-based authentication::
In this method, REST clients can create a session with cookies and pass these cookies in the header when making an API call. If you are accessing the REST API outside a web browser, create a long-lived session object in your code, and then call the login API  using that session object. You can use this object to send the session cookies in the subsequent REST API calls.

[NOTE]
====
When using the REST API v2 through a web browser, use either OAuth 2.0 bearer token authentication or trusted authentication to authenticate to ThoughtSpot and authorize your API requests. 
====
...
To sign in to ThoughtSpot, send a `POST` request to the `/tspublic/rest/v2/session/login` API endpoint.
...
To revoke the access token granted for the currently logged-in user, send a `POST` request to the `/tspublic/rest/v2/session/revoketoken` endpoint.
...
[#trustedAuthToken]
== Get login token for trusted authentication

To authenticate and log in using the trusted authentication method, you must obtain a login token from the ThoughtSpot application server. After you receive the login token, you can use it for logging in to ThoughtSpot and pass it in the `Authorization` header when sending an API request.

In the trusted authentication method, the login token is necessary during the login process, after which ThoughtSpot establishes a user session using cookies.
...
Note the login token and pass it in the authorization header when sending an API request.
