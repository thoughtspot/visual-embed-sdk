...
[IMPORTANT]
====
The REST API SDK does not support cookie-based authentication. Clients using the REST API SDK libraries can use only OAuth bearer tokens or trusted authentication. 
====
...
To authenticate and log in using the trusted authentication method, you must obtain a login token from the ThoughtSpot application server. After you receive the login token, you must use it in the Authorization header when sending an API request to log in to ThoughtSpot.
