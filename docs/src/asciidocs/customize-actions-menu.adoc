...
* Access control
+
Custom actions are visible to all users by default. However, only those users who have edit access to an answer or worksheet can view, edit, or assign a custom action to a visualization. For granular access control, you can associate a custom action to one or several user groups, and allow only users from these groups to access and use the custom action. To restrict access to a specific group, you must click *Show advanced availability settings* and select one or several user groups.  
