...
[%collapsible]
====
* Did you select the *On by default on all visualizations* checkbox when creating a custom action? If yes, the action will appear in the **More** image:./images/icon-more-10px.png[the more options menu] menu. If you want to add it to the contextual menu, edit the position of the action by using the edit icon in the *Custom actions* panel.
...
[NOTE]
====
By default, custom actions are visible only to those users who created the actions. To make the custom action available to your end users, the action creator must select the user group in *Show Advanced Availability Settings* and allow access.
====
