...
|[tag orangeBackground]#MODIFIED#|The SDK introduces `Passthrough` AuthType enumeration. The `AuthType.Passthrough` declaration is equivalent to `AuthType.None`, because both these options enable Embedded SSO authentication. +
...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#|The `detectCookieAccessSlow` parameter in the SDK allows your app to check if third-party cookies are enabled on the browser. This parameter is available only for `TrustedAuth` and `Basic` authentication types.
|====
...
This feature is applicable to token-based authentication, that is, when the `AuthType` is set as `TrustedAuth` in the SDK.
