...
|[tag greenBackground]#NEW FEATURE#|If trusted authentication is enabled, the `AuthType.AuthServer` now makes a `POST` API call to get a login token and log the user into ThoughtSpot.
