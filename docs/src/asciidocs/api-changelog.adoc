...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW# |The new version of the SDK provides the `SearchBarEmbed` JavaScript package to embed only the ThoughtSpot Search bar in your app.
+
For more information, see xref:embed-searchbar.adoc[Embed ThoughtSpot search bar].

|[tag greenBackground]#NEW# |The `customCSS` property in the `customizations` object supports new variables.
+
For more information, see xref:css-customization.adoc[Customize CSS].
|[tag greenBackground]#NEW# |The upcoming version of the SDK allows fetching TML objects via `Get TML` host event. This event is triggered when a user clicks on the *Show underlying data* action on a Liveboard visualization or answer page.
|[tag greenBackground]#NEW# a| The upcoming version of the SDK adds the following enums to the `Action` object:
