...
|[tag greenBackground]#NEW FEATURE#| The new version of the SDK supports firing events from the host application when a menu action on a Liveboard is triggered.
|====
...
|====
|[tag greenBackground]#NEW FEATURE#|
The new version of Visual Embed SDK includes the `liveboardV2` attribute in the `LiveboardEmbed` package to allow developers to enable the new Liveboard experience on their embedded ThoughtSpot instance. +
