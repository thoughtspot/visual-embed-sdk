...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE#  a| The `AddRemoveColumns` event is now available as `EmbedEvent` in in the SDK. This event is triggered when a column is selected.  

////
For more information, see xref:embed-events.adoc#embed-events[Interact with events].
////
|====
...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE# a|The `locale` attribute is now available in embed packages. You can use this attribute to set the locale or language of your embedded application view. 
////
...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE# a| The Visual Embed React library now includes the `useEmbedRef` hook, using which you can trigger events on ThoughtSpot components embedded in a React app. 
////
