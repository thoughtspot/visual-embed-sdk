...
[width="100%" cols="1,4"]
|====
|[tag greenBackground]#NEW FEATURE# |The new version of the SDK provides the `SearchBarEmbed` JavaScript package to embed only the ThoughtSpot Search bar in your app.
+
...
|[tag greenBackground]#NEW FEATURE# |The upcoming version of the SDK allows fetching TML objects via `GetTML` host event. This event is triggered when a user clicks on the *Show underlying data* action on a Liveboard visualization or answer page. +
