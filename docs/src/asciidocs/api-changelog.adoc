...
|[tag redBackground]#BREAKING CHANGE# a| The new Liveboard experience mode introduces changes in the data structure of the JSON response payload triggered by callback custom actions. For example, the `reportBookData`, and `vizData` metadata contents are modified, and the custom action properties are now part of the `embedAnswerData` object. These changes may break your current custom action event handlers. We recommend including `payload.data.id` in your code to handle callback custom action events as shown in this example. You may also want to update the data classes in your scripts to process the JSON response payload.
...
To view the data structure of the response payload, see the xref:callback-response-payload.adoc#_liveboard_new_experience[sample response payload].
