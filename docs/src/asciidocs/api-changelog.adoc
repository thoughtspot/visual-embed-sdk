...
|[tag greenBackground]#NEW FEATURE#| Support for advanced style customization with custom CSS [beta betaBackground]^Beta^
The new version of the SDK introduces the `customizations` property, which allows you to specify a custom CSS URL and define style customization variables in the `init` function. For more information, see xref:css-customization.adoc[Customize CSS].
|====
