...
ThoughtSpot allows you to add a custom action to trigger a data payload to a specific URL target. For example, you may search in ThoughtSpot for a specific data set and want to programmatically deliver this data to an external application or web page. ThoughtSpot Developer portal allows developers to add a custom menu item in the visualization pages and trigger a data push workflow when a user clicks the action.
...
* If a URL action is intended to be used by the embedded ThoughtSpot user, add the URL to the xref:security-settngs.adoc[CORS and CSP connect-src allowlist].
