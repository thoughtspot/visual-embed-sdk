...
[NOTE]
====
Starting with 8.9.0.cl, the `pinboard` object name appears as `liveboard` in the TML files exported from ThoughtSpot. The older version of TML files with `pinboard` can be imported into ThoughtSpot instances running 8.9.0.cl, but ThoughtSpot  replaces `pinboard` with `liveboard` when exporting TML.
