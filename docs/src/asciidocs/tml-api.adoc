...
|`formattype`|__String__. The format in which to export the objects. Valid values are `JSON` and `YAML`.
...
By default, the `export_associated` attribute is set to `false`.
a|`export_fqn` [tag greenBackground]#NEW in 8.7.0.cl# a|__Boolean__. When set to `true`, the API exports the TML representation of the specified objects with the `fqn` property. By default, the `export_fqn` attribute is set to `false`. +

If the TML file has multiple objects with the same name, the `name` property is not sufficient to identify the referenced object. Therefore, ThoughtSpot recommends adding the `fqn` property before importing the TML files into the system. The `fqn` property acts as a unique identifier and helps the system in identifying the referenced objects without any ambiguity. Note that the FQN of an object is the same as the GUID of the object. +

When you set the `export_fqn` attribute to `true`, the API returns the object data with `fqn` IDs, which you can use instead of manually adding the FQNs to the TML file before importing it into ThoughtSpot.

[IMPORTANT]
====
When you try to xref:tml-api.adoc#import[import] multiple objects in bulk and create objects using the `force_create` attribute, the new object may refer to another object with the same `fqn` in the ThoughtSpot system, instead of the object in the uploaded file. For example, if you have uploaded the TML representation of a Liveboard and related worksheet with the FQNs obtained from the TML export API, the imported Liveboard object may refer to a different worksheet with the same FQN in the ThoughtSpot system than the one imported in the zip file. If you intend to `force_create` objects during the TML import, you can set `export_fqn` to `false`.
|======
