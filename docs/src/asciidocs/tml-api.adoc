...
* If you intend to import multiple objects in bulk and create objects using the `force_create` attribute during xref:tml-api.adoc#import[TML import], set the `export_fqn` to `false`, so that there is no invalid or incorrect reference in the TML objects imported into ThoughtSpot.
|======
