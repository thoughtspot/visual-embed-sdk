...
[NOTE]
====
You can import single or multiple objects using the `tml/import` API. When you import a worksheet object, it may take a few seconds for the worksheet to become available in the ThoughtSpot system. Therefore, after you import a worksheet, wait for a few seconds to create answers and pinboards. 
However, if you have imported a worksheet along with pinboards, answers, and other dependent objects in a single API call, the imported objects will be immediately available for use.
====
