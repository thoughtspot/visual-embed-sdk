...
[NOTE]
====
You can import single or multiple objects using the `tml/import` API. When you import a worksheet object, it may take a few seconds for the worksheet to become available in the ThoughtSpot system. We recommend that you wait for a few seconds to create answers and Liveboards after you import a worksheet. 
If you have imported worksheets, Liveboards, answers, and other dependent objects in a single API call, the imported objects will be immediately available for use. 
====
