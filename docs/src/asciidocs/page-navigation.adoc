...
The response from the function above is an array of header objects, which can be parsed to render navigation.
...
The above function merely creates the table, it does not place it into the page. You can continue chaining using `.then()` to place the table in the appropriate place on your web application page :
...
If you want to navigate to a specific application page without initiating reload, you can set the `noReload` attribute to `true` as shown here:
