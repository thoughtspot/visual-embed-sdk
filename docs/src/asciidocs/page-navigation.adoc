...
Also note the `loadContent()` function referenced in the anchor tag created for the name column in the function above is a placeholder representing whatever is necessary to load that type of ThoughtSpot content in the web application. Loading content is discussed in the next section, but the actual design you choose for your application will determine the code you need to go from the navigation component to loading the ThoughtSpot content.
...
In general, most web applications will put the ThoughtSpot GUID into the URL in a way that allows the page to be bookmarked.
