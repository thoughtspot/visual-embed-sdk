...
|`topic`|__String__. Type of the log. Valid value is `security_logs`. 
|====
...
|`fromEpoch` __Optional__|__Epoch time string__. The EPOCH time in milliseconds to set the start time for streaming logs. For example, to set timestamp as June 1, 2021 8 am, specify `1622534400000`.
