...
////
[tag greyBackground]#-#  SAML authentication configuration +
...
////
On a multi-tenant cluster with Orgs, ThoughtSpot supports local and trusted authentication methods. If you are using Visual Embed SDK to embed ThoughtSpot in your app, use `AuthType.Basic` for local authentication and `AuthType.TrustedAuthToken` for trusted authentication. For more information, see xref:embed-authentication.adoc[Authentication].
...
The `/tspublic/v1/session/auth/token` also allows defining the Org context to which the user must be logged in after successful authentication. If the user should be logged in to a different Org context, you can specify the Org ID in the `orgid` property and set `autocreate` to `true`.
