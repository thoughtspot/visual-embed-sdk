...
ThoughtSpot supports setting up your cloud application instance to a single-tenant or multi-tenant cluster. In the single-tenant mode, ThoughtSpot lets you manage all of your organization's objects, data, users, and groups from a single workspace. In multi-tenant mode, a ThoughtSpot cloud application instance can be partitioned into multiple logical tenant-specific workspaces called *Orgs*. The Orgs feature allows you to set boundaries within the ThoughtSpot system and isolate each tenant's data in such a way that it is independent of and invisible to other tenant workspaces configured on same the application instance.
...
[options='header']
|=====
|Feature category|Supported operations at the cluster (All Org) level|Supported Operations at the Org level +
...
|Authentication a| [tag greenBackground]#✓#  Local authentication +

[tag greenBackground]#✓#  SAML authentication setup +

[tag greenBackground]#✓#  OIDC authentication setup +
__SAML and OIDC group synchronization features are not supported at the All Orgs level__ a| [tag greenBackground]#✓#  Local authentication setup +

[tag greenBackground]#✓#  Trusted authentication (Embedded instances) +

__For just-in-time (JIT) user provisioning and dynamic group assignment, ThoughtSpot recommends using the Trusted Authentication method.__+
...
|Data connections and objects a| [tag greenBackground]#✓# Data connection creation and management +

[tag greenBackground]#✓# Object creation and management a|[tag greenBackground]#✓#  Data connection creation and management +
...
[tag greenBackground]#✓#  Object sharing with others
...
|Link customization for embedded instances| [tag greyBackground]#–# |__The Link customization feature is supported only at the Primary Org (Org 0).__
...
__Group provisioning and custom action association API operations are not supported__.
...
If you are using SAML SSO to authenticate the embedded application users, you must configure the `orgs` attribute in the SAML authentication profile on ThoughtSpot to map the user to groups. To configure SAML authentication support for Org mapping, contact ThoughtSpot Support.
...
[IMPORTANT]
====
ThoughtSpot doesn't support Mapping SAML groups to ThoughtSpot groups on a multi-tenant cluster. Therefore, we recommend using xref:trusted-authentication.adoc[Trusted authentication], which supports just-in-time user creation, dynamic group mapping, and privilege assignment.
====
...
[NOTE]
=====
Before you get started with Orgs, make sure you read the documentation and understand the feature limitations of a multi-tenant setup.
=====
...
+
If the Orgs feature is enabled on a multi-tenant cluster, a Primary Org is created by default, and you will be logged in to the Primary Org context.
. Create Orgs in the *Admin* page of the UI or via xref:org-api.adoc#createOrg[REST API].
+
To create and manage Orgs, you must set the Org context to `All Orgs`. To do this, you can switch to All Orgs tab in the Admin page of UI or pass the ALL_ORGS scope as the `orgId` your API requests to Org endpoints. For more information, see xref:org-api.adoc[Org API].
. Create users and map the users to Orgs.
+
You can create an administrator profile for each Org and let the Org administrators manage users, groups, and role privileges in their respective Orgs.
Note that ThoughtSpot allows provisioning groups only within the context of an Org. You must ensure that ThoughtSpot users are mapped to appropriate Orgs and groups within these Orgs for user access control and data security.