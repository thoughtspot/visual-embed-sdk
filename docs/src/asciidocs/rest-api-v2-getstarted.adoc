...
To access the REST API v2 Playground, go to **Develop** > **REST API** > **REST Playground v2**. You can view a list of available endpoints on the left pane.

The Playground allows you to make API calls, view the request and response workflows, and and download samples.
...
To provide secure access to ThoughtSpot resources, the REST API v2 framework supports OAuth 2.0 Bearer token authentication and authorization flow. To make calls to the REST API v2 endpoints, you must obtain an OAuth access token and authorize your API requests. 
...
[NOTE]
====
By default, the generated token is valid for the specified duration. If an API request is made with an expired token, the server returns the `401 Authorization` code. You can request for a new access token and use it in your subsequent API calls.  
====
...
The REST API v2 framwork supports standard HTTP response codes to indicate the success or failure of a request. 
...
If you are making an API call in the Playground, to view the error details, go to `extensions` > `upstreamResponse` > `data` > `debug` in the error response body.

To view the response code, go to `extensions` > `upstreamResponse` > `status` in the error response body.
