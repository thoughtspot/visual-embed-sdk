...
To provide secure access to ThoughtSpot resources, the REST API v2 framework supports the OAuth 2.0 Bearer token authentication and authorization flow. To make calls to the REST API v2 endpoints, you must obtain an OAuth access token and pass it in the `Authorization` header to authorize your API requests. 

To get an OAuth access token, send a `POST` request to the `/api/rest/v2/session/gettoken` endpoint.
