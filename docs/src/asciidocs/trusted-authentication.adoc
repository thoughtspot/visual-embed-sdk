...
The trusted authentication workflow involves several entities such as the web browser, host application, authenticator server, and the ThoughtSpot server.
...
* Returns the token through the user's web browser
...
For token-based authentication method, ThoughtSpot administrators must xref:trusted-authentication.adoc#trusted-auth-enable[enable *Trusted authentication*] in the *Develop* > *Customization* > *Security Settings* page of the ThoughtSpot UI. This setting allows ThoughtSpot administrators to obtain a `secret key` that can be used when requesting access to ThoughtSpot on a user’s behalf.
...
. The SDK uses the token and username in the GET request to the `/tspublic/v1/session/login/token` endpoint.
