...
[NOTE]
====
If your ThoughtSpot instance is a multi-tenant environment with Orgs feature enabled, you must specify the `orgid` in the login token request.
For more information about Orgs and Org ID, see xref:orgs.adoc[Multi-tenancy with Orgs] and xref:session-api.adoc#session-loginToken[Authenticate and log in a user].
====

=== User creation and group assignment requests
