...
You can choose to create the user in ThoughtSpot the first time they request a login token. Starting from 8.9.0.cl, the xref:session-api.adoc#session-authToken[session/auth/token] for just-in-time provisioning of users. If the user for whom you are obtaining a token does not exist in the ThoughtSpot system, you can create a user and assign that user to groups by using `autocreate` and `group` attributes respectively.

[NOTE]
====
The xref:session-api.adoc#session-authToken[session/auth/token] API endpoint does not support creating new groups. The groups must already be configured on ThoughtSpot to add the user to groups.
====
...
* If the user should belong to other groups, issue the appropriate xref:group-api.adoc#add-user-to-group[Add User to Groups REST API request].
2. If the user doesn't exist in ThoughtSpot, set the `autocreate` property to `true` and specify the group names in your POST request to the xref:session-api.adoc#session-authToken[session/auth/token] API endpoint. +
 If the user already exists, compare the `assignedGroups` property for the user with the groups they should belong to.
