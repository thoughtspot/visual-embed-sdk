...
2. If the user already exists, compare the `assignedGroups` property for the user with the groups they should belong to.
2. If the user doesn't exist (Error Response), set the `autocreate` property to `true` in the API call to the `/tspublic/v1/session/auth/token` endpoint.
...
5. Request a login token from ThoughtSpot via `/tspublic/v1/session/auth/token` API endpoint.
...
1. Make a REST API request to get the xref:user-api.adoc#get-user-detail[user details] and xref:group-api.adoc#get-ug-details[group details for the user].
+
If the user already exists, compare the `assignedGroups` property for the user with the groups they should belong to.
2. If the user doesn't exist in ThoughtSpot, you can either xref:user-api.adoc#create-user[create a new user] or set the `autocreate` property to `true` in your API call to the xref:session-api.adoc#session-authToken[/tspublic/v1/session/auth/token] endpoint. You can also assign the user to groups in the same API call.
3. If the user should belong to other groups, you can add the user to groups via a REST API call to the xref:session-api.adoc#session-authToken[/tspublic/v1/session/auth/token] endpoint or use the xref:user-api.adoc#assignUserToGroups[/tspublic/v1/user/{userid}/groups] endpoint.

[NOTE]
====
The xref:session-api.adoc#session-authToken[/tspublic/v1/session/auth/token] API does not support creating new groups. The groups must already be configured on ThoughtSpot to add a user to groups.
====
