...
* When a new token is generated using the same secret key, the previous token will expire after five minutes.
* When a new secret key is generated for the ThoughtSpot server and the first new login token is obtained using the new secret key, all tokens generated using the previous secret key become invalid.
...
To set a consistent five minute expiration time, you can generate a second token to start the expiration clock on the previous login token that is sent to the user's browser.
