...
If you are using `EmbedEvent.VizPointClick` to trigger actions such as updating runtime filters on a Liveboard visualization, the click events from the new contextual menu may impact your app’s current workflow. To avoid this issue, use the following settings in the Visual Embed SDK from v1.21.0 and later versions:
