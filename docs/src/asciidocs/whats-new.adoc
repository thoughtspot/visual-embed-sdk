...
Starting from ThoughtSpot 7 Cloud October release, the *Custom actions* feature will be available for all ThoughtSpot cloud service users. You _do not_ require a separate ThoughtSpot Everywhere Edition license to create, manage, and assign custom actions to pinboard visualizations, charts and tables on clusters that have a valid ThoughtSpot Enterprise Cloud service subscription. You may also notice that the *Custom action* command under the *Customization* menu in the *Develop* tab no longer shows a locked icon. 

For more information about custom actions, see xref:custom-actions.adoc[Custom actions].

|[tag redBackground]#Removed# a|+++<h5>Removal of Only allow in context menu checkbox</<h5> +++

The *Only allow in context menu* checkbox in the custom action creation dialog is removed from the UI. This checkbox was available in previous releases to allow users to make the newly created custom action appear only in the contextual menu for charts and tables.

If you have configured a __global__ custom action to appear only in the context menu, it will now show as a menu action under the **More** image:./images/icon-more-10px.png[the more options menu] menu. To place this action on a context menu, click the edit icon and select **Context menu**.
