...
The upcoming release introduces the ability to add URL query parameters to the Liveboard, saved answer, and visualization links generated for an embedded ThoughtSpot instance. For example, you can customize the *Unsubscribe* link format to include query parameters that allow users to unsubscribe from monitor alerts and turn off email notifications.
...
|[tag redBackground]#BREAKING CHANGES# a| * The method names for the following endpoints in the SDK will be changed. +
