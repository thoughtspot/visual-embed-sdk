...
The version control API endpoints now allow you to generate a mapping file that records the GUIDs of the TML objects. The GUID mapping file is essential when your deployment environments have the same TML representation of objects but with different object GUIDs. It maps the GUIDs of the TML objects deployed on a production environment to the object GUIDs on the source environment.
...
On multi-tenant clusters with Orgs, developers can now add a list of CORS hosts at the Org level. The domain URLs allowed for CORS at the Org level take precedence over the CORS settings configured at the cluster level.
