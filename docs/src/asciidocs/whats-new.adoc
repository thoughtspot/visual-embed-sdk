...
If you are using `EmbedEvent.VizPointClick` to trigger actions such as updating runtime filters on a Liveboard visualization, the click events from the new contextual menu may impact your app’s current behavior. To avoid this issue, use the following settings in the Visual Embed SDK from v1.21.0 and later versions:

* Set contextual menu preference to right-click
+
...
* Use the `VizPointRightClick` event to monitor right-click menu actions
+
If you have set the contextual menu preference to `right-click` in the `ContextMenuTrigger` property, you can register the `EmbedEvent.VizPointRightClick` event to listen to right-click menu actions. You can use this event to customize the workflow based on the type of action selected from the contextual menu.
