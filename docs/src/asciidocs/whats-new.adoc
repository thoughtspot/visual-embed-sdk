...
Starting from ThoughtSpot 7 Cloud October release, the *Custom actions* feature will be available for all ThoughtSpot cloud service users. You _do not_ require a separate ThoughtSpot Everywhere Edition license to create and manage custom actions on clusters that have a valid ThoughtSpot Enterprise Cloud service subscription. You may also notice that the *Custom action* command under the *Customization* menu in the *Develop* tab no longer displays a locked icon and is available for users with Developer or Admin privileges.

|[tag redBackground]#BREAKING CHANGE# a|+++<h5>Removal of the Only allow in context menu checkbox</<h5> +++

The *Only allow in context menu* checkbox in the custom action creation dialog is removed from the UI. This checkbox was available in previous releases to allow users to set the custom action to appear only in the contextual menu on pinboard visualizations, charts, or tables. 

Note the following changes in the custom action configuration workflows:

* If you have a *Global* custom action that was configured to appear only in the contextual menu in earlier releases, it will show as a menu action under the **More** image:./images/icon-more-10px.png[the more options menu] menu. 

*  If you have a *Local* custom action that was configured to appear only in the contextual menu and was assigned to visualization, chart, or table in the earlier releases, the custom action is placed in the contextual menu. 

*  If you have a *Local* custom action that was configured to appear only in the contextual menu, but was not assigned to any visualization, chart, or table in earlier releases, the custom action does not show up in the contextual menu.

You can modify the position of custom actions by using the *Edit* option in the *Custom actions* panel displayed on a visualization page, chart, or table. For more information, see xref:custom-actions-edit.adoc[Set the position of a custom action].
|====
...
The ThoughtSpot 7 Cloud October release includes the following new endpoints. For more information about these APIs, see xref:rest-api-reference.adoc[REST API Reference].
