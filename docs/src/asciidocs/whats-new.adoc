...
== November 2021, _Version ts8.nov.cl_
...
As part of the rebranding, we have made some terminology changes in the ThoughtSpot UI and Developer portal. We are in the process of rolling out the terminology changes across all ThoughtSpot interfaces, platforms, and information artifacts. During this period, your environment may show some instances of `pinboard` based on the rebranding rollout stage. In some cases, we may continue to use the legacy terminology for backward compatibility, to ensure that your existing integrations work seamlessly. To familiarize yourself with terminology changes and the impacted interfaces, see the xref:terminology-update.adoc[Terminology changes] article.
|====
...
ThoughtSpot now allows you to restrict the custom action visibility to only users that belong to a specific group. If you have developer or admin privileges, you can configure the custom action availability settings in the Developer portal, so that only the authorized users can view and access the custom actions available for worksheets, answers, and Liveboard visualizations.

For more information, see custom actions.
|====

==== Visual Embed SDK version 1.6.0 
...
You can now define a list of UI actions that can be made visible to application users. If your embedded instance requires only a few actions, you can use the `visibleActions` API to add only those actions to the embedded UI.
...
The SDK library and object parameter names are modified to rebrand pinboards as Liveboards. For a complete list of changes, see xref:terminology-update.adoc[Terminology changes]. 
...
|====
||
|[tag greenBackground]#NEW API# a|`POST /tspublic/v1/session/login/token` 

This API allows you to securely send the login token and user information in the request body.

|[tag orangeBackground]#MODIFIED# a|`POST /tspublic/v1/connection/create`

The `/tspublic/v1/connection/create` now allows creating an empty connection without tables.
