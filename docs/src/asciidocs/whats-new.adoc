...
If your app uses `EmbedEvent.VizPointClick` to listen to click events on an embedded Liveboard visualization, once you activate the Cross-filters feature, you will notice that the `VizPointClick` event is triggered when a user clicks a contextual menu action, such as *Drill down* or *Filter*.
...
* No new endpoint, collections or resources will be introduced in the REST API v2 [beta betaBackground]^Beta^ version.
* The new REST v2.0 API endpoints are not an extension of the REST API v2 [beta betaBackground]^Beta^ endpoints. The resource categories, base path, and endpoint URIs are not the same as those in REST API v2 [beta betaBackground]^Beta^ version.
