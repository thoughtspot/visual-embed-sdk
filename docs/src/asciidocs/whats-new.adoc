...
New parameter for disabling filter actions::

The `pinboardEmbed` SDK package now includes the `preventPinboardFilterRemoval` parameter. You can use this parameter to disable filter-specific actions, such as editing or removing a filter applied to a visualization. 
...
+
For more information about `searchOptions`, see xref:embed-search.adoc[Embed ThoughtSpot search].
