...
Any user with developer or admin privileges can create an app action for Slack in the Developer portal, and insert it as a primary button or menu action on visualizations and saved answers in the ThoughtSpot UI. On clicking this action, ThoughtSpot users can initiate the app integration workflow and send data to their Slack channels without leaving the ThoughtSpot UI. 
...
The SDK now supports the `UpdateRuntimeFilters` host event. For more information, see xref:embed-events.adoc[Subscribe to events].
|====
