...
ThoughtSpot now offers the ThoughtSpot Everywhere Edition license that grants full access to ThoughtSpot Everywhere features such as the Developer portal, Playground, customization workflows, Visual Embed SDK, and REST APIs. If you have a ThoughtSpot Enterprise Cloud Service subscription and you want to avail the benefits of ThoughtSpot Everywhere, you can upgrade your license.

The ThoughtSpot Developer Portal displays a locked icon next to the customization menu actions on clusters that _do not_ have the ThoughtSpot Everywhere Edition license. When you clicked the locked icon, ThoughtSpot prompts you to sign up for a free trial or upgrade your license. 

* The Live Playground users can sign up for a free trial and evaluate ThoughtSpot Everywhere features.
...
* If you click *Upgrade* or **Upgrade Now**, ThoughtSpot opens the Live Chat Support widget. You can start a conversation with the Sales personnel and initiate the license upgrade.  
...
* The *Full app* Playground page includes the *Hide profile and help* checkbox to provide a preview of the `disableHelpAndProfile`  function that can hide the Help and Profile icons in the navigation bar.
...
The ThoughtSpot 7 Cloud September release supports Visual Embed SDK version 1.4.0, which includes the following  features and enhancements.
...
The `prefetch` API  fetches static resources from a given URL before your application loads. Web browsers can cache the prefetched resources locally and serve them from a user's local disk. You can use this API for the faster loading of embedded objects. 

|[tag greenBackground]#New feature# a|+++<h5>+++In-app page navigation+++</h5>+++

The `navigateToPage` method in the SDK lets you provide quick and direct access to a specific pinboard, saved answer, or an application page. You can add a custom menu action or button in your application UI to call the `navigateToPage` method and lead your users to the page specified in the `path` parameter.
...
* The `disableProfileAndHelp` attribute to show or hide the Help (?) and the user profile menu in the navigation bar of your embedded app. If you have embedded full ThoughtSpot application with the naviagtion bar, the Help (?) and the user profile menu icons are enabled by default.

* The `hideObjects` attribute to hide specific objects from a users' page view.  

For more information, see xref:full-embed.adoc[Embed full application].

|[tag greenBackground]#New feature# |+++<h5>+++Search embed +++</h5>+++

The `searchEmbed` package includes the `forceTable` attribute that sets the tabular view as the default format for presenting search data. You can use set this attribute to `true` to force search results to display in the table view. 
...
|===
||
|[tag greenBackground]#New endpoints# a| 
...
|[tag orangeBackground]#Modified#  a| * `POST /tspublic/v1/metadata/assigntag`
|===
