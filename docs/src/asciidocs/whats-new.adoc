...
The upcoming release allows query parameters in the Liveboard, saved answer, and visualization URLs that are customized for an embedded ThoughtSpot instance. For example, you can customize the *Unsubscribe* link sent in email notifications for KPI charts by adding `ts-query-params` to the visualization URL and thus allow users to unsubscribe from monitor alerts at any time.
