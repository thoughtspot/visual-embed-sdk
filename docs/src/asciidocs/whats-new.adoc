...
* The link:https://docs.thoughtspot.com/cloud/latest/liveboard-filters[Liveboard filter configuration options, window=_blank] are now available in a single modal.
* When a user creates a copy of a Liveboard, the filters applied to its visualizations are also copied

Other features and enhancements::
* The *Add filters* action is now placed on the primary menu bar and can be viewed only when a Liveboard is in edit mode. Only users with edit access to the Liveboard can apply filters.
...
* Improved link:https://docs.thoughtspot.com/cloud/latest/answer-explorer[visualization explore experience, window=_blank].
...
==== New Liveboard experience on embedded instances
The new Liveboard experience will be turned off by default on embedded ThoughtSpot instances. If you are using the Visual Embed SDK to embed ThoughtSpot, you can xref:embed-pinboard.adoc[set the `liveboardV2` parameter] in the LiveboardEmbed SDK package to `true` to enable the new experience globally for all users on your instance.

ThoughtSpot users with administrator privileges can also link:https://docs.thoughtspot.com/cloud/latest/liveboard-experience-new[turn on the new Liveboard experience, window=_blank] at the cluster level or let individual users choose their preferred experience.

[NOTE]
====
The Liveboard experience setting in the SDK takes precedence over the cluster-level settings in the *Admin* tab or user preference set in the profiles page in the UI.
====

==== Actions and Events in the SDK
If you have enabled the new Liveboard experience on your instance, you can use the `Action` and `Event` enumeration members available in the `LiveboardEmbed` SDK package to customize an embedded object and improve interactivity.
