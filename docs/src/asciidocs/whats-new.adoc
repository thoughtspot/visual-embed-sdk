...
Starting from ThoughtSpot 7 Cloud October release, the *Custom actions* feature will be available for all ThoughtSpot cloud service users. You _do not_ require a separate ThoughtSpot Everywhere Edition license to create or manage custom actions on clusters that have a valid ThoughtSpot Enterprise Cloud service subscription. You may also notice that the *Custom action* command under the *Customization* menu in the *Develop* tab no longer displays a locked icon and is available for users with Developer or Admin privileges.

|[tag redBackground]#BREAKING CHANGE# a|+++<h5>Removal of the Only allow in context menu checkbox</<h5> +++

The *Only allow in context menu* checkbox in the custom action creation dialog is removed from the UI. This checkbox was available in previous releases to allow users to set a custom action to appear only in the contextual menu on pinboard visualizations, charts, or tables. 

Note that if you have a *Global* custom action that was configured to appear only in the contextual menu in earlier releases, it will now show as a menu action under the **More** image:./images/icon-more-10px.png[the more options menu] menu. You can modify the position of the *Global* custom action by using the *Edit* option in the *Custom actions* panel on a visualization page, chart, or table. For more information, see xref:custom-actions-edit.adoc[Set the position of a custom action].
