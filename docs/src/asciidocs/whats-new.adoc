...
If a TML file on your source environment contains GUIDs that don't match the GUIDs on your destination environment, the version control APIs automatically handle the mapping of GUIDs when deploying the TML content.
...
`<object-name>.<guid>.tml` format. The earlier releases used the `<guid>.<object-name>.tml` naming convention for TML files. We recommend that you reconfigure your Git connection and start using the version control feature from scratch.
