...
Any user with developer or admin privileges can create an app action for Slack in the Developer portal, and add it as a primary button or menu action on visualizations and saved answers in the ThoughtSpot UI. On clicking this action, ThoughtSpot users can initiate the app integration workflow and send data to their Slack channels without leaving the ThoughtSpot UI.
...
=== Visual Embed SDK version 1.8.0

[width="100%" cols="1,4"]
|====
||
|[tag greenBackground]#NEW FEATURE# a| +++<h5>Action enumerations</h5> +++ 

The SDK introduces the following new enumerations for the embedded UI actions:
...
|[tag greenBackground]#NEW FEATURE#  a|+++<h5>Host events</h5>+++

The SDK now supports the `UpdateRuntimeFilters` host event. For more information, see xref:embed-events.adoc#host-events[Subscribe to events].
|====
...
|====
||
|[tag greenBackground]#NEW FEATURE# a|+++<h5>API endpoints</h5> +++ 
