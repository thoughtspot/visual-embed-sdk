...
If you want your application users to authenticate to an OpenID provider and use their SSO credentials to access the embedded ThoughtSpot content, you can set up OpenID Connect authentication on ThoughtSpot and enable the `OIDC` authentication type in the SDK. 
...
=== REST API
|====
|[tag orangeBackground]#NEW FEATURE# |+++<h5> REST API Playground and SDK </h5>+++ 

The REST API v2 Playground and SDK is a limited availability feature and is in beta. 

* If you are not a ThoughtSpot user, you can preview this feature on link:https://try-everywhere.thoughtspot.cloud/v2/#/everywhere/[ThoughtSpot Live Playground,  window=_blank]. 
* If you are a evaluating user, you can sign in to ThoughtSpot trial instance and explore the APIs and Playground.
* If you are an existing ThoughtSpot user, but do not have ThoughtSpot Everywhere Edition License, contact your ThoughtSpot Sales personnel to obtain a license.
* If you are an existing ThoughtSpot Everywhere user with a valid ThoughtSpot Everywhere Edition license, contact ThoughtSpot Support to enable the REST API  Playground and SDK feature on your cluster.
|====
