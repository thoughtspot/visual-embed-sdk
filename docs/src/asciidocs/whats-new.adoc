...
You can now set up your embedded ThoughtSpot instance to support OpenID connect authentication. The Visual Embed SDK 1.7.0 includes the `OIDC`  authentication method to enable OIDC authentication on embedded ThoughtSpot instances.

For more information, see xref:configure-oidc.adoc[OpenID Connect authentication].

|[tag redBackground]#NEW FEATURE# a| +++<h5>Follow button on the Liveboards page</h5> +++ 

Starting from 8.0.0.cl release, the *Follow* button on the Liveboards page is replaced with **Schedule**. You can now use  **Schedule** to schedule a Liveboard job.
|====

|====
|[tag greenBackground]#NEW FEATURE# |+++<h5> REST API Playground</h5> +++ 
|<To be added>.                                                                                                      
