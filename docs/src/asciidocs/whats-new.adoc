...
* If you have a *Global* custom action with the *Only allow in context menu* setting enabled, the configuration setting _is not_ preserved and the actions are placed in the **More** image:./images/icon-more-10px.png[the more options menu] menu instead of the contextual menu. 
+
You can modify the position of this action by using the *Edit* option in the *Custom actions* panel on a visualization or search results page. For more information, see xref:custom-actions-edit.adoc[Set the position of a custom action].

* If you have a *Local* custom action with the *Only allow in context menu* setting enabled, and the action is already assigned to a visualization, chart, or table, the configuration setting _is_ preserved and the custom action shows up in the contextual menu.

* If your instance has a *Local* custom action with the *Only allow in context menu* setting enabled and it is not assigned to any visualization, chart, or table, the action will not be available as a menu item or button. You must add the action to a visualization, chart, or table, and modify its position as required.
