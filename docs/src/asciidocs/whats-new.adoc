...
If you are using `EmbedEvent.VizPointClick` to trigger actions such as updating runtime filters on a Liveboard visualization, the click events from the new contextual menu may impact your app’s current behavior. To minimize this impact, ThoughtSpot provides the following options in the upcoming Visual Embed SDK versions (v1.21.0 and later):
