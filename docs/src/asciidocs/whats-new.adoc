...
If your deployment uses `EmbedEvent.VizPointClick` to listen to click events on a Liveboard visualization or Answer on your embedded instance, note that the embedded object will now emit the `VizPointClick` event when a user clicks a contextual menu action such as Drill down, Filter, and so on.
