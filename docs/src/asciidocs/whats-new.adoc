...
Starting with 9.0.0.cl, the REST API v2[beta betaBackground]^Beta^ API endpoints are deprecated and removed from the REST API v2 Playground. The REST API v2 API Playground will include a set of new endpoints generally available (GA) on all ThoughtSpot instances.

With the new version of the REST API v2 endpoints, you can perform more tasks in a single API call. For example, you can create a new user, map the user to groups and Orgs, set home Liveboards for the user, and assign privileges in a single API request.
...
* The new REST v2 API endpoints are not an extension of the REST v2 API [beta betaBackground]^Beta^ endpoints. The resource categories, base path, and endpoint URIs are not the same as those of the REST API v2 [beta betaBackground]^Beta^ version.
* Some API operations for data connections, Liveboard schedules, and Orgs are not available in the initial release. For more information, see xref:rest-api-v2-reference.adoc[REST API v2 reference].
====
...
In the upcoming release, your administrators can enable the following Early Access features:
