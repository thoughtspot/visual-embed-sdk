...
The new licensing model introduces the following workflows in the ThoughtSpot UI:

* By default, the ThoughtSpot Developer Portal displays a locked icon for the customization features on clusters that do not have the ThoughtSpot Everywhere Edition license. When you click the locked icon, the UI displays a page that prompts you to sign up for a free trial or upgrade your license.

* The ThoughtSpot Enterprise Cloud service subscribers can start a 30-day free trial or initiate a license upgrade request from the UI.
