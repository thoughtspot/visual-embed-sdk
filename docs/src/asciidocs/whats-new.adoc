...
ThoughtSpot now allows you to restrict the custom actions to specific user groups. Developers can configure the custom action availability settings in the Developer portal, so that only the authorized users can view and access the custom actions available for worksheets, answers, and Liveboard visualizations.
...
You can now configure a set of ThoughtSpot UI actions as visible actions and display only these actions in the embedded ThoughtSpot UI. If your embedded instance requires only a few actions, you can use the `visibleActions` API to quickly add the actions that you want to show in your application UI.
