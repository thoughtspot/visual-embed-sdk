...
ThoughtSpot now allows you to restrict a custom action availability to specific user groups. Developers can configure the custom action availability settings in the Developer portal, so that only the authorized users can view and access the custom actions available for worksheets, answers, and Liveboard visualizations.
