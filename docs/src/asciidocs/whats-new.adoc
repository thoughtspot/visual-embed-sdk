...
ThoughtSpot users with edit privileges can now define or modify the position of a custom action on visualization pages. When developers create a custom action in the Developer portal, ThoughtSpot adds a menu item to the **More** image:./images/icon-more-10px.png[the more options menu] menu by default. Developers can explicitly set a custom action as a context menu in the Developer portal or allow ThoughtSpot users to modify the position of the  custom action on a visualization page.
