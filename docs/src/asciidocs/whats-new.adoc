...
If your deployment uses `EmbedEvent.VizPointClick` to listen to click events on an embedded Liveboard visualization, note that the embedded object will now emit the `VizPointClick` event when a user clicks a contextual menu action such as *Drill down*, *Filter*, and so on.
