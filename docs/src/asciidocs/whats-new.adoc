...
The ThoughtSpot 7 Cloud July release supports Visual Embed SDK version 1.3.0, which includes the major and minor enhancements.  
...
+
The `searchQuery` parameter in the `searchEmbed` SDK package is deprecated in the Visual Embed SDK version 1.3.0. You can use the `searchOptions` parameter to define the search token string.
...
The JSON data structure passed in custom action response for search answers now shows as  `payload.data.embedAnswerData`. The payload now includes the following metadata:
