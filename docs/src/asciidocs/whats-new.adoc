...
Starting with 9.0.0.cl, the REST API v2[beta betaBackground]^Beta^ API endpoints are deprecated and removed from the REST API v2 Playground. The REST API v2 API Playground will include a set of new endpoints that will be generally available (GA) on all ThoughtSpot instances.
...
* The new REST v2.0 API endpoints are not an extension of the REST v2 API [beta betaBackground]^Beta^ endpoints. The resource categories, base path, and endpoint URIs are not the same as those of the REST API v2 [beta betaBackground]^Beta^ version. The v2.0 version of API endpoints may break your current implementation REST API v2 [beta betaBackground]^Beta^ endpoints.
...
ThoughtSpot administrators can enable the following Early Access features on your instances:
