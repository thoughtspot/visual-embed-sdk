...
By default, the contextual menu in ThoughtSpot is set as right-click pop-up menu. To trigger the contextual menu on  right-click or left-click, you can use the `ContextMenuTrigger` property in the Visual Embed SDK.
...
==== Turn off cross filters on an embedded instance

Use the `Action.CrossFilter` and `Action.RemoveCrossFilter` parameters in the SDK to hide or disable cross filters on your embedded app. For more information, see xref:embed-actions.adoc[Show or hide menu items] and xref:embed-action-ref.adoc#liveboardv2-viz-actions[Action reference].
