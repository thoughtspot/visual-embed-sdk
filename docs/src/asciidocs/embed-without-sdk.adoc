...
* To get the embed link for a Liveboard from the UI: +
...
* To get the embed link for a visualization from the UI: +
...
For example, to filter values equal to `michigan` in the `State` column for a visualization on a Liveboard, you can pass the runtime filter in the URL query parameters as shown here:
